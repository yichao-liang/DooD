#!/bin/bash  

# sbatch execute.sh python sweep.py --beta 2.5 -m emM --seed 0
# sbatch execute.sh python sweep.py --beta 2.5 -m emM --seed 1
# sbatch execute.sh python sweep.py --beta 2.5 -m emM --seed 2
# sbatch execute.sh python sweep.py --beta 2.5 -m emM --seed 3
# sbatch execute.sh python sweep.py --beta 2.5 -m emM --seed 4

# sbatch execute.sh python sweep.py --beta 2.5 -m kmM --seed 0
# sbatch execute.sh python sweep.py --beta 2.5 -m kmM --seed 1
# sbatch execute.sh python sweep.py --beta 2.5 -m kmM --seed 2
# sbatch execute.sh python sweep.py --beta 2.5 -m kmM --seed 3
# sbatch execute.sh python sweep.py --beta 2.5 -m kmM --seed 4

# sbatch execute.sh python sweep.py --beta 2.5 -m mnM --seed 0
# sbatch execute.sh python sweep.py --beta 2.5 -m mnM --seed 1
# sbatch execute.sh python sweep.py --beta 2.5 -m mnM --seed 2
# sbatch execute.sh python sweep.py --beta 2.5 -m mnM --seed 3
# sbatch execute.sh python sweep.py --beta 2.5 -m mnM --seed 4

# sbatch execute.sh python sweep.py --beta 2.5 -m omM --seed 0
# sbatch execute.sh python sweep.py --beta 2.5 -m omM --seed 1
# sbatch execute.sh python sweep.py --beta 2.5 -m omM --seed 2
# sbatch execute.sh python sweep.py --beta 2.5 -m omM --seed 3
# sbatch execute.sh python sweep.py --beta 2.5 -m omM --seed 4

# sbatch execute.sh python sweep.py --beta 2.5 -m qdM --seed 0
# sbatch execute.sh python sweep.py --beta 2.5 -m qdM --seed 1
# sbatch execute.sh python sweep.py --beta 2.5 -m qdM --seed 2
# sbatch execute.sh python sweep.py --beta 2.5 -m qdM --seed 3
# sbatch execute.sh python sweep.py --beta 2.5 -m qdM --seed 4

# sbatch execute.sh python sweep.py --beta 4 -m mnCor --seed 0
# sbatch execute.sh python sweep.py --beta 4 -m mnCor --seed 1
# sbatch execute.sh python sweep.py --beta 4 -m mnCor --seed 2 
# sbatch execute.sh python sweep.py --beta 4 -m mnCor --seed 3 
# sbatch execute.sh python sweep.py --beta 4 -m mnCor --seed 4 

# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 5
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 6
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 7 
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 8 
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 9 

# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 10
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 11
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 12
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 13
# sbatch execute.sh python sweep.py --beta 4 -m mnIm --seed 14

sbatch execute.sh python sweep.py --beta 4 -m mnIm10 --seed 5 
sbatch execute.sh python sweep.py --beta 4 -m mnIm10 --seed 6
sbatch execute.sh python sweep.py --beta 4 -m mnIm10 --seed 7
sbatch execute.sh python sweep.py --beta 4 -m mnIm10 --seed 8
sbatch execute.sh python sweep.py --beta 4 -m mnIm10 --seed 9

# sbatch execute.sh python sweep.py --beta 4 -m mnBzR --seed 0
# sbatch execute.sh python sweep.py --beta 4 -m mnBzR --seed 1
# sbatch execute.sh python sweep.py --beta 4 -m mnBzR --seed 2 
# sbatch execute.sh python sweep.py --beta 4 -m mnBzR --seed 3 
# sbatch execute.sh python sweep.py --beta 4 -m mnBzR --seed 4 