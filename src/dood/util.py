...
from kornia.geometry.transform import get_affine_matrix2d, invert_affine_transform
...
from dood.datasets import cluster, multimnist, oneshot_omniglot, synthetic
from dood.models import air, base, ssp, vae  # , mws
...
def get_last_vars(hs, z_smpl):
    ...
    z_where = torch.gather(
        z_where, 2, idx[:, :, None, None].repeat(1, 1, 1, wr_dim)
    ).squeeze(0)
    z_what = torch.gather(
        z_what, 2, idx[:, :, None, None, None].repeat(1, 1, 1, npts, 2)
    ).squeeze(0)
...
def get_top_latents_hiddens(guide, gen, img, ptcs, crit="likelihood"):
    ...
    log_post_z = torch.cat(
        [prob.sum(-1, keepdim=True) for prob in log_post], dim=-1
    ).sum(-1)
    log_prior, log_lld = gen.log_prob(
        latents=latents, imgs=img, z_pres_mask=mask_prev, canvas=canvas, z_prior=z_prior
    )
    log_prior_z = torch.cat(
        [prob.sum(-1, keepdim=True) for prob in log_prior], dim=-1
    ).sum(-1)
    ...
    # select zs
    # z_pres [ps, bs, n_strk]
    tp_z_pres = torch.gather(latents.z_pres, 0, idx[None, :, None].repeat(1, 1, s))
    # z_where [ps, bs, n_strk, wr_dim]
    tp_z_where = torch.gather(
        latents.z_where, 0, idx[None, :, None, None].repeat(1, 1, s, w)
    )
    # z_what [ps, bs, n_strk, npts, 2]
    tp_z_what = torch.gather(
        latents.z_what, 0, idx[None, :, None, None, None].repeat(1, 1, s, p, 2)
    )
    tp_zs = ZSample(
        tp_z_pres,
        tp_z_what,
    ...
    tp_strk_tanh = torch.gather(dec_param[1][0], 0, idx[None, :, None].repeat(1, 1, s))
    tp_add_tanh = torch.gather(dec_param[1][1], 0, idx[None, :, None].repeat(1, 1, s))
    tp_dec_param = DecoderParam(sigma=tp_sigma, slope=(tp_strk_tanh, tp_add_tanh))
    # select reconstruction
    tp_rec = torch.gather(
        rec, 0, idx[None, :, None, None, None].repeat(1, 1, 1, 50, 50)
    )
...
def get_sample_affine(device):
    ...
    if large_sample:
        ...
        shear_horizontal_angles = torch.linspace(
            -0.2 * math.pi, 0.2 * math.pi, num_discretizations_l
        )
        ...
        shear_horizontal_angles = torch.linspace(
            -0.1 * math.pi, 0.1 * math.pi, num_discretizations
        )
        shear_vertical_angles = shear_horizontal_angles
        rotate_angles = shear_horizontal_angles
...
def init_dataloader(res, dataset, batch_size=64, rot=False, shuffle=True, args=None):
    # Dataloader
    # Train dataset
    if dataset in [
        "Omniglot",
    ...
    if dataset == "EMNIST":  # trn 697,932 tst 116,323
        ...
        trn_dataset, tst_dataset = synthetic.get_data_loader(
            args=args, make_new_dataset=True
        )
...
def init_classification_nets(guide, args, dataset, batch_size, trned_ite):
    ...
    # source dataset
    train_loader, test_loader, _, _ = init_dataloader(
        res, dataset, batch_size=batch_size
    )
    ...
    if args.save_model_name == "VAE":
        ...
        if only_z_what:
            ...
            classifier_in_dim = guide.max_strks * (guide.z_where_dim + guide.z_what_dim)
    classifier_out_dim = test_loader.dataset.num_classes

    lv_classifier = init_mlp(
    ...
    return (lv_classifier, (train_loader, test_loader), optimizer, stats)
...
def character_conditioned_sampling(n_samples, guide_out, decoder):
    ...
    canvas = torch.zeros(
        [n_samples, bs, 1, decoder.res, decoder.res],
    ...
    for t in range(n_strks):
        ...
        z_where_loc = out.z_smpl.z_where[:, :, t, :].expand(
            n_samples, bs, decoder.z_where_dim
        )
        ...
        z_what_loc = out.z_smpl.z_what[:, :, t, :, :].expand(
            n_samples, bs, decoder.pts_per_strk, 2
        )
        z_what_loc = sampling_gauss_noise(z_what_loc, var)

        # render step
        # canvas_step [n_smples, bs, 1, res, res]
        canvas_step = decoder.renders_imgs(
            (z_pres_p, z_what_loc.unsqueeze(2), z_where_loc.unsqueeze(2))
        )
        # update the canvas
        ...
        canvas = normalize_pixel_values(canvas, method="tanh", slope=add_slope)
        ...
        for n, p in guide.named_parameters():
            ...
            if n == name and p.grad.norm(2) > 1e4:
                ...
                for param_n, param in guide.named_parameters():
                    writer.add_scalar(
                        f"Grad_norm_debug/img{idx}/{param_n}",
                        param.grad.norm(2),
                        iteration,
                    )
                for loss_n, l in zip(loss_tuple._fields, loss_tuple):
                    writer.add_scalar(
                        f"Loss_debug/img{idx}/{loss_n}/", l.detach().mean(), iteration
                    )
...
def get_affines(logits):
    ...
    shift = constrain_parameter(logits[..., :2], -0.1, 0.1)
    # good for om
    # scale = constrain_parameter(logits[...,2:4], .8, 1.2)
    # good for qd
    scale = constrain_parameter(logits[..., 2:4], 0.9, 1.1)
    rot = constrain_parameter(logits[..., 4:5], -0.1 * math.pi, 0.1 * math.pi)
    shear = constrain_parameter(logits[..., 5:7], -0.1 * math.pi, 0.1 * math.pi)
    return torch.cat([shift, scale, rot, shear], dim=3)


def transform_z_what(z_what, z_where, z_where_type):
    """Apply z_where_mtrx to z_what such that it has a similar result as applying the
    inverse z_where_mtrx to the z_what-rendering.
    ...
    z_where_mtrx = get_affine_matrix_from_param(
        thetas=z_where.view(bs * n_strk, -1), z_where_type=z_where_type
    ).view(bs, n_strk, 2, 3)
    z_what = (z_what - 0.5) * 2

    homo_coord = torch.cat([z_what, torch.ones_like(z_what)[..., :1]], dim=3)
    transformed_z_what = (z_where_mtrx @ homo_coord.transpose(2, 3)).transpose(2, 3)
    transformed_z_what = transformed_z_what * 0.5 + 0.5
...
def init_z_where(z_where_type):
    """'3': (shift x, y, scale, ) # '4_no_rotate': (scale x, y, shift x, y) '4_rotate':

    (shift x, y, scale, rotate) '5': (shift x, y, scale x, y, rotate)
    """
    init_z_where_params = {
        "3": ZWhereParam(
            torch.tensor([0, 0, 0.8]), torch.tensor([0.2, 0.2, 0.2]), 3
        ),  # AIR?
        ...
        "4_rotate": ZWhereParam(
            torch.tensor([0, 0, 0.8, 0]), torch.tensor([0.2, 0.2, 0.2, 0.4]), 4
        ),
        # torch.ones(4)/5, 4),
        "5": ZWhereParam(
            torch.tensor([0, 0, 0.8, 0.8, 0]),
            torch.tensor([0.2, 0.2, 0.2, 0.2, 0.4]),
            5,
        ),
    }
...
def sigmoid(x, min=0.0, max=1.0, shift=0.0, slope=1.0):
...
def get_save_test_img_dir(args, epoch, prefix="reconstruction", suffix="tst"):
...
def get_checkpoint_path(args, checkpoint_iteration=-1):
    ...
    return get_checkpoint_path_from_path_base(
        get_path_base_from_args(args), checkpoint_iteration
    )
...
def init(run_args, device, init_data_loader=True):
    ...
    if init_data_loader:
        ...
        if run_args.model_type == "MWS":
            ...
            train_loader = mws.data.get_data_loader(
                trn_dataset.data / 255, run_args.batch_size, run_args.device, ids=True
            )
            test_loader = mws.data.get_data_loader(
                tst_dataset.data / 255,
                run_args.batch_size,
                run_args.device,
    ...
    if run_args.model_type == "Base":
        ...
        guide = ssp.Guide(
            ...
            feature_extractor_sharing=not run_args.no_feature_extractor_sharing,
            num_mlp_layers=run_args.num_mlp_layers,
        ...
        generative_model, guide, optimizer, memory, stats = mws.util.init(
            mws_args, device
        )
...
def init_optimizer(run_args, model):
    ...
    if run_args.model_type == "AIR":
        air_parameters = itertools.chain(
            guide.air_params(), generative_model.parameters()
        )
        optimizer = torch.optim.Adam(
            [
                {
        ...
        if run_args.use_canvas or run_args.prior_dist == "Sequential":
            if run_args.anneal_non_pr_net_lr:
                assert (
                    run_args.sep_where_pres_net != run_args.simple_pres
        ...
        if run_args.anneal_non_pr_net_lr:
            ...
            if run_args.dataset in ["EMNIST", "Omniglot", "KMNIST", "Quickdraw"]:
        ...
        torch.save(
            {
                ...
                "guide_state_dict": guide.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
                "schedular_state_dict": (
                    schedular.state_dict() if schedular != None else None
                ),
                "stats": stats,
                "run_args": run_args,
            },
...
def load_checkpoint(
    path, device, finetune_dataset_name=None, init_data_loader: bool = True
):
    scheduler = None
    try:
    ...
    model, optimizer, scheduler, stats, data_loader = init(
        run_args, device, init_data_loader
    )
...
Stats = collections.namedtuple(
    "Stats", ["trn_losses", "trn_elbos", "tst_losses", "tst_elbos"]
)
...
def conv_block(in_channels, out_channels, dropout=0.0, ks=3):
    block = nn.Sequential(
        nn.Conv2d(
            in_channels, out_channels, kernel_size=ks, stride=1, padding=1, bias=False
        ),
...
class SpatialTransformerNetwork(nn.Module):
    def __init__(self):
        ...
        self.localization2[2].bias = torch.nn.Parameter(
...
def get_affine_matrix_from_param(thetas, z_where_type, center=None):
    """Get a batch of 2x3 affine matrix from transformation parameters thetas of shape
    [batch_size, 4 (shift x, y; scale; angle)].
    ...
    if z_where_type == "4_rotate":
        ...
        # 1 rotate
        angle = thetas[:, 3]
        angle_cos, angle_sin = (torch.cos(angle), torch.sin(angle))
    ...
    affine_matrix = get_affine_matrix2d(
        translations=translations, center=center, scale=scale, angle=angle
    )[:, :2]
    return affine_matrix


def init_stn(
    in_dim=None, out_dim=None, num_mlp_layers=None, non_linearity=None, end_cnn=False
):
...
def max_normalize(imgs, max_per_img):
    ...
    imgs[(max_per_img != 0).squeeze()] = (
        imgs[(max_per_img != 0).squeeze()] / non_zero_max
    )
    return imgs
