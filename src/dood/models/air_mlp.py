...
class PresMLP(nn.Module):
    ...
    def __init__(self, in_dim, dataset=None):
        ...
        if dataset in ["KMNIST"]:
            init_bias = 4
        if dataset in ["Omniglot", "Quickdraw"]:
            # init_bias = 15
...
class PresWhereMLP(nn.Module):
    ...
    def __init__(self, in_dim, z_where_type, z_where_dim, dataset):
        ...
        self.seq = util.init_mlp(
            in_dim=in_dim, out_dim=1 + z_where_dim * 2, hidden_dim=256, num_layers=2
        )
        # init_bias = 4
        # if dataset == "Omniglot":
        #     init_bias = 20
    ...
    def forward(self, h):
        ...
        if self.type == "3":
            ...
            z_where_loc = torch.cat([z_where_shift_loc, z_where_scale_loc], dim=-1)

        elif self.type == "4_rotate":
            z_where_shift_loc = util.constrain_parameter(z[:, 1:3], min=-0.8, max=0.8)
            z_where_scale_loc = F.softplus(z[:, 3:4]) + 1e-6
            z_where_rot_loc = z[:, 4:5]
            z_where_loc = torch.cat(
                [z_where_shift_loc, z_where_scale_loc, z_where_rot_loc], dim=-1
            )
        else:
            raise NotImplementedError
        # to avoid 0 in scale
...
class Decoder(nn.Module):
    ...
    def forward(self, z_what):
        ...
        # exp: unnormalized
        out_loc = F.softplus(out + self.bias).view(*z_what.shape[:2], *self.img_dim)
        return out_loc
