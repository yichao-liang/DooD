...
class Guide(nn.Module):
    def __init__(
        ...
        if self.intr_ll is not None:
            assert use_canvas, "intermediate likelihood needs" + "use_canvas = True"
        ...
        if no_pres_rnn or no_rnn:
            assert sep_where_pres_net, (
                "need to have seperate where pres net"
        ...
        if simple_pres:
            assert use_residual, "simple_pres requires execution guide and residual"
        ...
        if feature_extractor_type == "MLP":
            ...
            self.img_feature_extractor = util.init_cnn(
                ...
                mlp_out_dim=self.feature_extractor_out_dim,
                mlp_hidden_dim=self.feature_extractor_out_dim,
                num_mlp_layers=1,
            )
            self.use_sep_trans_img_extractor = False
        ...
        if sep_where_pres_net:
            ...
            if not self.no_pres_rnn:
                self.pr_rnn = torch.nn.GRUCell(
                    self.pr_rnn_in_dim, self.pr_wr_rnn_hid_dim
                )

            self.wr_rnn = torch.nn.GRUCell(self.wr_rnn_in_dim, self.pr_wr_rnn_hid_dim)
        else:
            ...
            self.pr_wr_rnn_hid_dim = hidden_dim
            self.pr_wr_rnn = torch.nn.GRUCell(
                self.pr_wr_rnn_in_dim, self.pr_wr_rnn_hid_dim
            )

        # 2.2 pres_where_mlp
        if self.simple_pres:
            self.pr_rsd_power = torch.nn.Parameter(
                torch.zeros(1) + 5.0, requires_grad=True
            )
        if self.sep_where_pres_net:
            ...
            if self.target_in_pos == "MLP" and not self.residual_no_target:
                if not self.only_rsd_ratio_pres and not self.residual_no_target_pres:
        ...
        self.wt_rnn = torch.nn.GRUCell(self.wt_rnn_in_dim, self.wt_rnn_hid_dim)

        # 3.2 wt_mlp: instantiated in specific modules
        self.wt_mlp_in = []
        self.wt_mlp_in_dim = 0
        if not self.no_post_rnn and not self.no_rnn and not self.no_what_post_rnn:
            self.wt_mlp_in = ["h"]
        ...
        self.bl_regressor = util.init_mlp(
            in_dim=self.bl_hid_dim,
            out_dim=1,
    ...
    def print_model_statistics(self):
        ...
        print(
            f"## [{self.use_sep_trans_rsd_extractor}] "
            + "Use seperated rsd glimpse extractor"
        )
        print(
            f"## [{self.use_sep_trans_img_extractor}] "
        ...
        if self.sep_where_pres_net:
            ...
            if self.no_pres_rnn:
                ...
                print(
                    f"## z_pres rnn in {self.pr_rnn_in_dim}dim: " + f"{self.pr_rnn_in}"
                )
            print(f"## z_pres mlp in {self.pr_mlp_in_dim}dim: " + f"{self.pr_mlp_in}\n")
            print(f"## z_where rnn in {self.wr_rnn_in_dim}dim: " + f"{self.wr_rnn_in}")
            print(
                f"## z_where mlp in {self.wr_mlp_in_dim}dim: " + f"{self.wr_mlp_in}\n"
            )

        else:
            print("# Shared z_where, z_pres networks:")
            print(
                f"## z_pres_where rnn in {self.pr_wr_rnn_in_dim}dim: "
                + f"{self.pr_wr_rnn_in}"
            )
            print(
                f"## z_pres_where mlp in {self.pr_wr_mlp_in_dim}dim: "
                + f"{self.pr_wr_mlp_in}\n"
            )
    ...
    def get_img_features(self, imgs, canvas, residual):
        ...
        # embed image
        img_embed = self.img_feature_extractor(imgs.view(prod(shp), *img_dim)).view(
            *shp, -1
        )
        if self.use_canvas:
            ...
            if self.detach_canvas_embed:
                canvas_embed = canvas_embed.detach()

        if self.use_residual:
            if self.detach_rsd_embed:
                residual_embed = (
                    self.img_feature_extractor(residual.view(prod(shp), *img_dim))
                    .view(*shp, -1)
                    .detach()
                )
            else:
        ...
        if self.simple_pres or self.residual_pixel_count:
            ...
            rsd_ratio = torch.nan_to_num(rsd_ratio, nan=0.0)
            # print("rsd_ratio", rsd_ratio.isnan().any())

            if self.detach_rsd_embed:
                rsd_ratio = rsd_ratio.detach()

        return img_embed, canvas_embed, residual_embed, rsd_ratio

    def get_pr_wr_mlp_in(
        self, img_embed, canvas_embed, residual_embed, rsd_ratio, p_state
    ):
        ...
        if self.sep_where_pres_net:
            ...
            rnn_in = [
                p_state.z_pres.view(prod(shp), -1),
        ...
        if self.sep_where_pres_net:
            pr_mlp_in, wr_mlp_in = [], []
            if "h" in self.pr_mlp_in:
                pr_mlp_in.append(h_pr)
            if "h" in self.wr_mlp_in:
                wr_mlp_in.append(h_wr)
    ...
    def sample_pr_wr(self, p_state, z_pres_p, z_where_loc, z_where_scale, z_where_cor):
        """z_pres_p [ptcs, bs, -1] z_where_loc [ptcs, bs, z_where_dim] z_where_scale
        [ptcs, bs, z_where_dim] z_where_cor [ptcs, bs, 1]"""
        ...
        if self.correlated_latent:
            ...
            # cov[..., 0, 1] = z_where_cor
            z_where_post = MultivariateNormal(loc=z_where_loc, scale_tril=tril)
        else:
    ...
    def sample_wt(self, z_what_loc, z_what_std, z_what_cor, z_pres):
        """z_what_loc [ptcs, bs, pts_per_strk, 2] z_what_std [ptcs, bs, pts_per_strk, 2]
        ...
        if self.correlated_latent:
            ...
            z_what_post = Independent(
                MultivariateNormal(loc=z_what_loc, scale_tril=tril),
                reinterpreted_batch_ndims=1,
            )
        else:
            z_what_post = Independent(
                Normal(z_what_loc, z_what_std), reinterpreted_batch_ndims=2
            )
