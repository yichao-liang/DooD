...
class GenerativeModel(nn.Module):
    def __init__(self, hidden_dim=512, z_dim=8, res=50):
        ...
        self.decoder = Decoder(
            z_dim=z_dim, img_dim=[1, res, res], hidden_dim=hidden_dim, num_layers=2
        )
    ...
    def z_dist(self, bs=[1, 3]):
        ...
        dist = Independent(Normal(loc, std), reinterpreted_batch_ndims=1).expand(bs)
    ...
    def img_dist(self, latents=None):
        ...
        imgs_dist_loc, imgs_dist_std = (
            imgs_dist_loc.view(*shp, 1, self.res, self.res),
            imgs_dist_std.view(*shp, 1, self.res, self.res),
        )
        dist = Independent(
            Laplace(imgs_dist_loc, imgs_dist_std), reinterpreted_batch_ndims=3
        )
        assert dist.event_shape == torch.Size(
            [1, self.res, self.res]
...
class Guide(nn.Module):
    ...
    def forward(self, imgs, num_particles=1):
        ...
        z_post = Independent(Normal(z_loc, z_std), reinterpreted_batch_ndims=1)
        assert z_post.event_shape == torch.Size(
            [self.z_dim]
        ) and z_post.batch_shape == torch.Size([bs])
        ...
        data = GuideReturn(z_smpl=z, z_pms=(z_loc, z_std), z_lprb=z_lprb)
        return data
...
class Decoder(nn.Module):
    def __init__(self, z_dim=8, img_dim=[1, 50, 50], hidden_dim=256, num_layers=3):
        super().__init__()
        self.img_dim = img_dim
        self.z_dim = z_dim
        self.out_dim = np.prod(img_dim)
        self.net = util.init_mlp(
            in_dim=z_dim,
    ...
    def forward(self, zwhat):
        out = self.net(zwhat)
        out_loc = torch.sigmoid(out[:, : self.out_dim]).view(
            zwhat.shape[0], *self.img_dim
        )
        out_std = (
            F.softplus(out[:, self.out_dim :]).view(zwhat.shape[0], *self.img_dim)
            + 1e-6
        )
