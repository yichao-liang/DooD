...
def constrain_z_where(z_where_type, z_where_loc, clamp=False, more_range=False):
    ...
    if z_where_type == "5":
        z_where_shift_loc = constrain_f(z_where_loc[:, 0:2], min=-0.8, max=0.8)
        z_where_scale_loc = constrain_f(z_where_loc[:, 2:4], min=0.25, max=1)
        z_where_ang_loc = constrain_f(
            z_where_loc[:, 4:5], min=-np.pi / 4, max=np.pi / 4
        )
        ...
        z_where_shift_loc = constrain_f(z_where_loc[:, 0:2], min=-0.8, max=0.8)
        if more_range:
            z_where_scale_loc = constrain_f(z_where_loc[:, 2:3], min=0.25, max=1)
        else:
            z_where_scale_loc = constrain_f(z_where_loc[:, 2:3], min=0.25, max=1)
        ...
        if z_where_type == "4_rotate":
            z_where_ang_loc = constrain_f(
                z_where_loc[:, 3:4], min=-np.pi / 4, max=np.pi / 4
            )
...
def constrain_z_what(z_what_loc, clamp=False, more_range=False):
    ...
    if clamp:
        z_what_loc = torch.clamp(z_what_loc, min=-0.2, max=1.2)
    else:
        # safe
        if more_range:
            z_what_loc = util.constrain_parameter(z_what_loc, min=-0.2, max=1.2)
...
class GlobalRendererParamMLP(nn.Module):
    ...
    def __init__(
        ...
        self.seq = util.init_mlp(
            in_dim=in_dim, out_dim=1, hidden_dim=hidden_dim, num_layers=num_layers
        )
    ...
    def forward(self, h):
        ...
        if self.sgl_strk_tanh:
            ...
            add_slope = util.constrain_parameter(z, min=0.1, max=1.5)
        else:
...
class LocalRendererParamMLP(nn.Module):
    ...
    def __init__(
        ...
        self.trans_z_what = trans_z_what
        self.seq = util.init_mlp(
            in_dim=in_dim, out_dim=2, hidden_dim=hidden_dim, num_layers=num_layers
        )
        self.seq.linear_modules[-1].weight.data.zero_()
        ...
        if self.maxnorm and self.sgl_strk_tanh:
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                # works well with no canvas
                torch.tensor([b1, b2], dtype=torch.float)
            )
        elif not self.sgl_strk_tanh and not self.maxnorm:
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
    ...
    def forward(self, h):
        ...
        if self.trans_z_what:
            sigma = util.constrain_parameter(z[:, 0:1], min=0.005, max=0.04)
        else:
            sigma = util.constrain_parameter(z[:, 0:1], min=0.01, max=0.04)

        # stroke slope
        if self.maxnorm:
            sgl_strk_slope = util.constrain_parameter(
                z[:, 1:2], min=0.1, max=0.9
            )  # maxnorm
        else:
...
class PresMLP(nn.Module):
    def __init__(
        ...
        self.seq.linear_modules[-1].bias = torch.nn.Parameter(
            torch.tensor([init_bias], dtype=torch.float)
        )  # works for stable models
        ...
        if z_where_type == "4_rotate":
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    ...
                    [0, 0, init_b, 0, -4, -4, -4, -4, 0],  # loc  # std
                    dtype=torch.float,
                )
            )  # cor
        elif z_where_type == "5":
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    [0, 0, init_b, init_b, 0, -4, -4, -4, -4, -4, 0], dtype=torch.float
                )
            )
        elif z_where_type == "3":
            # [loc:shift,scale, std:scale,shift]
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor([0, 0, init_b, -4, -4, -4, 0], dtype=torch.float)
            )
        else:
            raise NotImplementedError
...
class PresWhereMLP(nn.Module):
    ...
    def __init__(
        ...
        if z_where_type == "4_rotate":
            ...
            # [pres,  loc:scale,shift,rot,  std:scale,shift,rot]
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor([4, 4, 4, 0, 0, 0, -4, -4, -4, -4, -4], dtype=torch.float)
            )
        elif z_where_type == "3":
            self.seq.linear_modules[-1].weight.data.zero_()
    ...
    def forward(self, h):
        ...
        # has minimal constrain
        z_where_loc = constrain_z_where(z_where_type=self.type, z_where_loc=z_where_loc)
        z_where_std = util.constrain_parameter(z_where_std, min=1e-9, max=1)
...
class WhatMLP(nn.Module):
    ...
    def forward(
        ...
        """z_what_loc [bs, pts_per_strk * 2] z_what_std [bs, pts_per_strk * 2]
        ...
        if self.constrain_param:
            z_what_loc = constrain_z_what(z_what_loc, more_range=self.more_range)

        # has minimal constrain (no constrain previously)
        z_what_std = torch.sigmoid(z_what_std) + 1e-9

        z_what_cor = util.constrain_parameter(out[:, -self.npts :], min=-1, max=1)
...
class PresPriorMLP(PresMLP):
    def __init__(self, in_dim, hidden_dim, num_layers, dataset):
        ...
        self.seq.linear_modules[-1].weight.data.zero_()
        self.seq.linear_modules[-1].bias = torch.nn.Parameter(
            torch.tensor([pres_b], dtype=torch.float)
        )
...
class WherePriorMLP(nn.Module):
        ...
        if z_where_type == "4_rotate":
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    ...
                    [0, 0, 4, 0]
                    + (
                        torch.tensor([0, 0, 4, 0] * (n_comp - 1))
                        + torch.randn(4 * (n_comp - 1)) * 0.001
                    ).tolist()
                    +
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    ...
                    + [0, 0, 4, 4, 0]
                    + (
                        torch.tensor([0, 0, 4, 4, 0] * (n_comp - 1))
                        + torch.randn(5 * (n_comp - 1)) * 0.001
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    ...
                    ([4] + (torch.rand(3 * n_comp - 1) * 0.6 - 0.3).tolist())
                    + [0] * 3 * n_comp,
                    dtype=torch.float,
                )
            )
    ...
    def forward(self, h):
        ...
        all_loc = z[:, self.n_comp : self.n_comp + (self.z_where_dim * self.n_comp)]
        ...
        all_std = z[:, self.n_comp + (self.z_where_dim * self.n_comp) : -self.n_comp]
        all_std = util.constrain_parameter(
            all_std.reshape(bs * self.n_comp, -1), min=1e-9, max=1
        ).view(bs, self.n_comp, -1)
        # [bs, n_comp]
        all_cor = util.constrain_parameter(z[:, -self.n_comp :], min=-1, max=1)
        return logits, all_loc, all_std, all_cor


class PresWherePriorMLP(nn.Module):
    def __init__(
        self, in_dim, z_where_type, z_where_dim, hidden_dim, num_layers, n_comp, dataset
    ):
        super().__init__()
        self.z_where_dim = z_where_dim
        self.type = z_where_type
        ...
        if z_where_type == "4_rotate":
            ...
            self.seq.linear_modules[-1].bias = torch.nn.Parameter(
                torch.tensor(
                    [4]
                    + [0.1] * n_comp
                    + ([4] + (torch.rand(5 * n_comp - 1) * 0.6 - 0.3).tolist())
                    +
                    # ([4,4] + (torch.rand(3)*.6-.3).tolist())*n_comp +
                    [0] * 5 * n_comp,
                    dtype=torch.float,
    ...
    def forward(self, h):
        """Logits [bs, n_comp] all_loc [bs, n_comp, z_where_dim] all_cor [bs, n_comp]"""
        z = self.seq(h)
        bs = z.shape[0]
        ...
        all_loc = z[
            :, 1 + self.n_comp : 1 + self.n_comp + (self.z_where_dim * self.n_comp)
        ]
        all_loc = constrain_z_where(
            z_where_type=self.type,
            z_where_loc=all_loc.reshape(bs * self.n_comp, -1),
        ...
        all_std = z[
            :, 1 + self.n_comp + (self.z_where_dim * self.n_comp) : -self.n_comp
        ]
        all_std = util.constrain_parameter(
            all_std.reshape(bs * self.n_comp, -1), min=1e-9, max=1
        ).view(bs, self.n_comp, -1)
...
class WhatPriorMLP(nn.Module):
    ...
    def forward(self, h):
        ...
        all_loc = constrain_z_what(
            z[:, tot_comp : tot_comp + tot_comp * 2].reshape(bs * self.n_comp, -1),
            more_range=self.more_range,
        ).view(bs, pps, nc, 2)

        all_std = (
            torch.sigmoid(
                z[:, tot_comp + tot_comp * 2 : -tot_comp].reshape(bs * self.n_comp, -1)
            ).view(bs, pps, nc, 2)
            + 1e-9
        )

        all_cor = util.constrain_parameter(z[:, -tot_comp:], min=-1, max=1).view(
            bs, pps, nc
        )
...
class ImageMLP(nn.Module):
    def __init__(self, in_dim, out_dim, hid_dim, num_layers):
        super().__init__()
        self.mlp = util.init_mlp(
            in_dim=in_dim, out_dim=out_dim, hidden_dim=hid_dim, num_layers=num_layers
        )
...
PtRnnState = namedtuple("PtRnnState", "z_what h")


class ControlPointPriorRNN(nn.Module):
    def __init__(self, in_dim, pts_per_strk, hid_dim, n_comp, correlated_latent=True):
        super().__init__()
        ...
        self.mlp = WhatPriorMLP(
            in_dim=hid_dim, pts_per_strk=1, hid_dim=hid_dim, num_layers=2, n_comp=n_comp
        )
        self.init_h = torch.nn.Parameter(torch.zeros(self.hid_dim), requires_grad=True)
    ...
    def forward(self, x):
        ...
        device = x.device
        state = PtRnnState(
            z_what=torch.zeros(bs, 2).to(device), h=self.init_h.expand(bs, self.hid_dim)
        )
        pis, locs, stds, cors = [], [], [], []
