"""Attend, Infer, Repeat model Sequential model with distributed z_what latent
variables."""
...
from torch.distributions import Bernoulli, Independent, Laplace, Normal

from dood import util
from dood.models import template
from dood.models.air_mlp import *
...
class GenerativeModel(nn.Module):
    def __init__(
        ...
        if prior_dist == "Independent":
            ...
            # z_pres
            self.register_buffer("z_pres_prob", torch.zeros(self.max_strks) + 0.5)

            # z_where: default '3'
            self.register_buffer(
                "z_where_loc", z_where_loc.expand(self.max_strks, self.z_where_dim)
            )
            self.register_buffer(
                "z_where_std", z_where_std.expand(self.max_strks, self.z_where_dim)
            )
    ...
    def control_points_dist(self, h_c=None, bs=[1, 3]):
        ...
        if self.prior_dist == "Sequential" and h_c is not None:
            ...
            loc, std = self.z_what_loc.expand(
                *bs, self.z_what_dim
            ), self.z_what_std.expand(*bs, self.z_what_dim)
        else:
            raise NotImplementedError

        dist = Independent(Normal(loc, std), reinterpreted_batch_ndims=1)
    ...
    def img_dist(self, latents=None, canvas=None):
        ...
        assert dist.event_shape == torch.Size(
            [1, self.res, self.res]
        ) and dist.batch_shape == torch.Size([*shp])
        return dist
    ...
    def renders_imgs(self, latents):
        ...
        z_where_mtrx = util.get_affine_matrix_from_param(
            z_where.reshape(ptcs * bs * n_strks, -1), self.z_where_type
        )
...
class Guide(template.Guide):
    ...
    def forward(self, imgs, num_particles=1):
        ...
        for t in range(self.max_strks):
            ...
            assert (
                ...
                and state.z_what.shape == torch.Size([ptcs, bs, self.z_what_dim])
            ...
            if self.prior_dist == "Sequential":
                ...
                z_pres_prir[:, :, t] = (
                    self.internal_decoder.presence_dist(h_l, [*shp]).log_prob(
                        z_pres_smpl[:, :, t].clone()
                    )
                    * mask_prev[:, :, t].clone()
                )
    ...
    def inference_step(self, p_state, imgs, canvas, residual):
        ...
        pr_wr_mlp_in, h_l = self.get_pr_wr_mlp_in(
            img_embed, canvas_embed, residual_embed, residual, p_state
        )
        z_pres, z_where, z_pres_lprb, z_where_lprb, z_pres_p, z_where_pms = (
            self.get_z_l(pr_wr_mlp_in, p_state)
        )
        ...
        z_what, z_what_lprb, z_what_pms = self.get_z_c(wt_mlp_in, p_state, z_pres)
    ...
    def get_z_l(self, pr_wr_mlp_in, p_state):
        ...
        if self.sep_where_pres_net:
            ...
            z_where_loc, z_where_scale = self.wr_mlp(pr_wr_mlp_in.view(prod(shp), -1))
        else:
        ...
        z_pres_post = Independent(Bernoulli(z_pres_p), reinterpreted_batch_ndims=1)
        assert z_pres_post.event_shape == torch.Size(
            [1]
        ) and z_pres_post.batch_shape == torch.Size([*shp])
        ...
        z_where_post = Independent(
            Normal(z_where_loc, z_where_scale), reinterpreted_batch_ndims=1
        )
        assert z_where_post.event_shape == torch.Size(
            [self.z_where_dim]
        ) and z_where_post.batch_shape == torch.Size([*shp])
    ...
    def get_z_c(self, wt_mlp_in, p_state, z_pres):
        ...
        z_what_post = Independent(
            Normal(z_what_loc, z_what_std), reinterpreted_batch_ndims=1
        )
        assert z_what_post.event_shape == torch.Size(
            [self.z_what_dim]
    ...
    def initialize_state(self, imgs, ptcs):
        ...
        z_what_smpl = torch.zeros(
            ptcs, bs, self.max_strks, self.z_what_dim, device=imgs.device
        )
        ...
        z_what_pms = torch.zeros(
            ptcs, bs, self.max_strks, self.z_what_dim, 2, device=imgs.device
        )
