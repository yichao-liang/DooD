...
def schedule_model_parameters(gen, guide, iteration, loss, device):
    ...
    if loss == "elbo":
        if iteration == 0:
            ...
            # gen.sigma = torch.log(torch.tensor(.04))
            # doesn't work well
            gen.control_points_scale = (
                torch.ones(gen.strks_per_img, gen.ctrl_pts_per_strk, 2) / 5
            ).to(device)
        # if iteration == 2e3:
        #     gen.sigma = torch.log(torch.tensor(.03))
    elif loss == "nll":
        ...
        if iteration == 0:
            gen.sigma = torch.log(torch.tensor(0.02))
            gen.control_points_scale = (
                torch.ones(gen.strks_per_img, gen.ctrl_pts_per_strk, 2) / 5
            ).to(device)
...
class GenerativeModel(nn.Module):
    def __init__(
        ...
        """In the base model we pre-specify the number of control points and work with
        data that has 1 stroke.
        ...
        if prior_dist == "Normal":
            self.register_buffer(
                "control_points_loc",
                torch.zeros(self.strks_per_img, self.ctrl_pts_per_strk, 2) + 0.5,
            )
            self.register_buffer(
                "control_points_scale",
                torch.ones(self.strks_per_img, self.ctrl_pts_per_strk, 2) / 5,
            )
            # divide by 5 works
        ...
        """For thicker stroke, and blur control `sigma` is for passing in the renderer,
        should make sure it's positive.
        ...
        """
        # Renderer parameter
        self.sigma = torch.nn.Parameter(torch.tensor(6.0), requires_grad=True)
        ...
        self.tanh_norm_slope = torch.nn.Parameter(torch.tensor(6.0), requires_grad=True)
        ...
        if self.prior_dist == "Normal":
            return torch.distributions.Independent(
                torch.distributions.Normal(
                    self.control_points_loc, self.control_points_scale
                ),
            ...
            return torch.distributions.Independent(
                torch.distributions.Uniform(self.uniform_low, self.uniform_high),
                reinterpreted_batch_ndims=2,
            )
        else:
            raise NotImplementedError
    ...
    def img_dist_b(
        ...
        if self.prior_dist == "Dirichlet":
            ...
            control_points_b = control_points_b.chunk(2, -1)[0].reshape(
                batch_dim, 1, -1, 2
            )

        if self.norm_pixel_method == "maxnorm":
            sigma_min, sigma_max = 0.01, 0.05
        elif self.norm_pixel_method == "tanh":
            sigma_min, sigma_max = 0.01, 0.05

        imgs_dist_loc = self.bezier(
            control_points_b,
            # sigma=torch.exp(self.sigma)) # positive
            sigma=util.constrain_parameter(self.sigma, min=sigma_min, max=sigma_max),
        )

        # max normalized so each image has pixel values [0, 1]
        imgs_dist_loc = util.normalize_pixel_values(imgs_dist_loc, method="maxnorm")
        # useful with maxnorm normalize
        if self.norm_pixel_method == "maxnorm":
            imgs_dist_loc = dilation(imgs_dist_loc, self.dilation_kernel, max_val=1.0)
        elif self.norm_pixel_method == "tanh":
            # the slope of the tanh experiment can be further tuned.
            imgs_dist_loc = util.normalize_pixel_values(
                imgs_dist_loc,
                method=self.norm_pixel_method,
                slope=util.constrain_parameter(self.tanh_norm_slope, min=0.1, max=0.7),
            )
        ...
        if self.stn_transform is not None:
            imgs_dist_loc = util.inverse_stn_transformation(
                imgs_dist_loc, self.stn_transform
            )
            imgs_dist_loc = util.normalize_pixel_values(
                imgs_dist_loc, method=self.norm_pixel_method
            )

        assert not imgs_dist_loc.isnan().any()
        assert imgs_dist_loc.max() <= 1.0
        ...
        dist = torch.distributions.Independent(
            event_dist(imgs_dist_loc, imgs_dist_std), reinterpreted_batch_ndims=3
        )
        return dist
    ...
    def log_prob(self, latents, imgs):
        ...
        if (self.prior_dist == "Dirichlet") and (
            latents.shape[-2] * 2 == self.control_points_dist_b(shape).event_shape[-2]
        ):
            """This happens when e.g. we use a *Normal inference dist* with output size
            [b, strks_per_img, n_points, 2 (for x, y)] with a *Dir prior dist* which has
            shape [b, strks_per_img, n_points*2 (for x, y), 2 (which adds up to 1)]."""
            latents = latents.view(
                [*shape, self.strks_per_img, self.ctrl_pts_per_strk * 2, 1]
            )
            latents = torch.cat([latents, torch.ones_like(latents) - latents], dim=-1)
...
class Guide(nn.Module):
    ...
    def get_control_points_dist(self, imgs):
        ...
        if self.dist == "Normal":
            ...
            concentration = torch.sigmoid(self.control_points_net(imgs)).view(
                *[*shape, self.strks_per_img, self.ctrl_pts_per_strk * 2, 2]
            )
            return torch.distributions.Independent(
