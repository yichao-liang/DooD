...
from dood.models import air_mlp, template
from dood.models.ssp_mlp import *
from dood.models.template import GenState, GuideState, ZLogProb, ZSample
from dood.plot import save_img_debug as sid
from dood.splines.bezier import Bezier

# latent variable tuple
DecoderParam = namedtuple("DecoderParam", "sigma slope")
GuideReturn = namedtuple(
    "GuideReturn",
    [
        "z_smpl",
        "z_lprb",
...
GenReturn = namedtuple("GenReturn", ["z_smpl", "canvas"])
...
class GenerativeModel(nn.Module):
    def __init__(
        ...
        if self.intr_ll == "Geom":
            self.intr_ll_geo_p = torch.nn.Parameter(
                torch.tensor(-10.0), requires_grad=True
            )

        if self.intr_ll is not None:
            assert self.use_canvas, (
                "intermediate likelihood needs" + "use_canvas = True"
            )
        ...
        if prior_dist == "Independent":
            if self.fixed_prior:
                ...
                # z_what
                self.register_buffer("pts_loc", torch.zeros(self.pts_per_strk, 2) + 0.5)
                if generate_data:
                    self.register_buffer(
                        "pts_std", torch.zeros(self.pts_per_strk, 2) + 0.4
                    )
                    self.register_buffer("z_pres_prob", torch.zeros(1) + 1.0)
                    self.register_buffer(
                        "z_where_loc", z_where_loc.expand(1, self.z_where_dim)
                    )
                    self.register_buffer(
                        "z_where_std", z_where_std.expand(1, self.z_where_dim)
                    )
                else:
                    self.register_buffer(
                        "pts_std", torch.zeros(self.pts_per_strk, 2) + 0.2
                    )
                    # z_pres
                    self.register_buffer(
                        "z_pres_prob", torch.zeros(self.max_strks) + 0.5
                    )
                    self.register_buffer(
                        "z_where_loc",
                ...
                self.pts_loc = torch.nn.Parameter(
                    torch.zeros(self.pts_per_strk, 2) + 0.5, requires_grad=True
                )
                self.pts_std = torch.nn.Parameter(
                    torch.ones(self.pts_per_strk, 2) / 5, requires_grad=True
                )
                self.z_pres_prob = torch.nn.Parameter(
                    torch.zeros(self.max_strks) + 0.99, requires_grad=True
                )
                z_where_loc, z_where_std, self.z_where_dim = util.init_z_where(
                    self.z_where_type
                )
                self.z_where_loc = torch.nn.Parameter(z_where_loc, requires_grad=True)
                self.z_where_std = torch.nn.Parameter(z_where_std, requires_grad=True)
            ...
            raise NotImplementedError
        self.imgs_dist_std = torch.nn.Parameter(
            torch.zeros(1, res, res), requires_grad=True
        )
        ...
        if self.input_dependent_param:
            ...
            self.sigma = torch.nn.Parameter(torch.tensor(6.0), requires_grad=True) / 3
            self.sgl_strk_tanh_slope = torch.nn.Parameter(
                torch.tensor(6.0), requires_grad=True
            )
            self.add_strk_tanh_slope = torch.nn.Parameter(
                torch.tensor(6.0), requires_grad=True
            )
    ...
    def get_intr_ll_geo_p(self):
        return util.constrain_parameter(self.intr_ll_geo_p, min=0.5, max=1.0 - 1e-6)

    def get_sigma(self):
        return util.constrain_parameter(self.sigma, min=0.01, max=0.04)

    def get_add_strk_tanh_slope(self):
        return util.constrain_parameter(self.add_strk_tanh_slope, min=0.1, max=0.7)

    def get_sgl_strk_tanh_slope(self):
        return util.constrain_parameter(self.sgl_strk_tanh_slope, min=0.1, max=0.7)
    ...
    def presence_dist(self, h_l=None, bs=[1, 3], pri_wt=None, t=0, sample=False):
        """
        ...
        if self.prior_dist == "Sequential":
            ...
            if self.no_pres_rnn or self.no_rnn:
                ...
                if pri_wt is not None and self.prior_dependency == "wr|wt":
                    mlp_in = torch.cat(
                        [h_l.view(prod(bs), -1), pri_wt.view(prod(bs), -1)], dim=-1
                    )
                # if self.sep_where_pres_net:
            ...
            if not self.fixed_prior:
                z_pres_p = util.constrain_parameter(
                    z_pres_p, min=1e-12, max=1 - (1e-12)
                )
        else:
            raise NotImplementedError
        self.z_pres_p = z_pres_p
        dist = Independent(
            Bernoulli(z_pres_p.view(*bs)),
            reinterpreted_batch_ndims=0,
        )
        assert dist.event_shape == torch.Size([]) and dist.batch_shape == torch.Size(
            [*bs]
        )

        return dist

    def transformation_dist(self, h_l=None, bs=[1, 3], pri_wt=None, sample=False):
        ...
        if self.prior_dist == "Sequential":
            ...
            if self.correlated_latent:
                ...
                comp = Independent(Normal(loc, std), reinterpreted_batch_ndims=1)
            dist = MixtureSameFamily(mix, comp)

        elif self.prior_dist == "Independent":
            loc, std = self.z_where_loc.expand(
                *bs, self.z_where_dim
            ), self.z_where_std.expand(*bs, self.z_where_dim)
            if not self.fixed_prior:
                loc = constrain_z_where(
                    z_where_type=self.z_where_type, z_where_loc=loc.squeeze(0)
                )
        ...
        assert dist.event_shape == torch.Size(
            [self.z_where_dim]
        ) and dist.batch_shape == torch.Size([*bs])

        return dist

    def control_points_dist(self, h_c=None, bs=[1, 3], pri_wr=None, sample=False):
        ...
        if self.prior_dist == "Sequential" and h_c is not None:
            ...
            if pri_wr != None and self.prior_dependency == "wt|wr":
                mlp_in = torch.cat(
                    [h_c.view(prod(bs), -1), pri_wr.view(prod(bs), -1)], dim=-1
                )
            ...
            if self.correlated_latent:
                ...
                if sample or self.constrain_var:
                    ...
                    cov = tril @ tril.transpose(-1, -2)
                    # cov = cov * .01
                    tv = torch.gather(cov[..., 0, 0], 2, pi.max(2)[1].unsqueeze(-1))
                    tc = torch.gather(cov[..., 0, 1], 2, pi.max(2)[1].unsqueeze(-1))
                ...
                dist = Independent(
                    MixtureSameFamily(mix, comp), reinterpreted_batch_ndims=1
                )
            else:
            ...
            loc, std = self.pts_loc.expand(
                *bs, self.pts_per_strk, 2
    ...
    def img_dist(self, latents=None, canvas=None):
        ...
        dist = Independent(
            base_dist(imgs_dist_loc, imgs_dist_std), reinterpreted_batch_ndims=3
        )
        ...
        assert dist.event_shape == torch.Size(
            [1, self.res, self.res]
        ) and dist.batch_shape == torch.Size(batch_size)
        return dist

    def renders_imgs(self, latents):
        """Batched img rendering.

        Decode z_what then transform accroding to
        ...
        if self.transform_z_what:
            ...
            imgs = self.decoder(
                trans_what, sigma.view(prod(shp), n_strks, -1), keep_strk_dim=True
            )
            ...
            z_where_mtrx = util.get_affine_matrix_from_param(
                z_where.view(ptcs * bs * n_strks, -1), self.z_where_type
            )
        ...
        if self.maxnorm:
            imgs = util.normalize_pixel_values(
                imgs,
                method="maxnorm",
            )
        ...
        if self.sgl_strk_tanh:
            ...
            imgs = util.normalize_pixel_values(
                imgs, method=self.norm_pixel_method, slope=slope
            )
    ...
    def renders_glimpses(self, z_what):
        ...
        if self.spline_decoder:
            recon = self.decoder(
                z_what.view(ptcs * bs, n_strks, n_pts, 2),
        ...
        if self.sgl_strk_tanh:
            if self.input_dependent_param:
                ...
                slope = self.get_sgl_strk_tanh_slope()
            recon = util.normalize_pixel_values(recon, method="tanh", slope=slope)
        recon = recon.view(ptcs, bs, n_strks, 1, res, res)
        return recon
    ...
    def log_prob(
        self, latents, imgs, z_pres_mask, canvas, decoder_param=None, z_prior=None
    ):
        """
        Args:
            latents:
        ...
        if z_prior is not None and z_prior[0] is not None:
            ...
            log_prior = ZLogProb(
                z_pres=(self.presence_dist(bs=bs).log_prob(z_pres) * z_pres_mask),
                z_what=(self.control_points_dist(bs=bs).log_prob(z_what) * z_pres),
                z_where=(self.transformation_dist(bs=bs).log_prob(z_where) * z_pres),
            )
        ...
        log_likelihood = self.img_dist(latents=latents, canvas=canvas).log_prob(imgs)
        ...
        if self.intr_ll == "Mean":
            ...
            log_likelihood_[num_steps != 0] = (
                log_likelihood[num_steps != 0] / num_steps[num_steps != 0].unsqueeze(1)
            ).sum(1)
    ...
    def sample(
        ...
        if self.prior_dist == "Sequential":
            ...
            z_what_smpl = torch.zeros(
                *bs, self.max_strks, self.pts_per_strk, 2, device=self.device
            )
            z_where_smpl = torch.ones(
                *bs, self.max_strks, self.z_where_dim, device=self.device
            )
            if init_canvas is None:
                canvas = torch.zeros(*bs, 1, self.res, self.res, device=self.device)
            else:
                canvas = init_canvas
            strk_img = None
            if linear_sum:
                strk_img = torch.zeros(
                    *bs, self.max_strks, 1, self.res, self.res, device=self.device
                )
            ...
            for t in range(max_steps):
                ...
                if linear_sum:
                    ...
                    if init_canvas != None:
                        canvas = torch.sum(
                            torch.cat([init_canvas, canvas], dim=1), dim=1, keepdim=True
                        )
                    add_slope = decoder_param.slope[1][0, :, 0]
                    canvas = util.normalize_pixel_values(
                        canvas, method="tanh", slope=add_slope
                    )
                else:
    ...
    def generation_step(self, p_state, canvas, init_h=False, t=0):
        ...
        if self.dependent_prior:
            if self.prior_dependency == "wt|wr":
                # [bs]
                z_pres = self.presence_dist(
                    h_l, [bs], pri_wt=None, t=t, sample=True
                ).sample()
                # [bs, where_dim]
                z_where = self.transformation_dist(
                    h_l, [bs], pri_wt=None, sample=True
                ).sample()
                pri_wr = z_where
                if self.condition_by_img:
                    glmps = util.spatial_transform(
                        ...
                        util.get_affine_matrix_from_param(
                            z_where.view(bs, -1), z_where_type=self.z_where_type
                        ),
                    )
                    # if t==1:
                    #     sid(glmps[0], 'gen_glimpse')
                    glmps_em = self.img_feature_extractor(glmps).view(
                        bs, -1
                    )  # .detach()
                    pri_wr = torch.cat([pri_wr, glmps_em], -1)
                # [bs, num_pts, 2]
                z_what = self.control_points_dist(
                    h_c, [bs], pri_wr=pri_wr, sample=True
                ).sample()
            elif self.prior_dependency == "wr|wt":
                z_what = self.control_points_dist(
                    h_c, [bs], pri_wr=None, sample=True
                ).sample()
                z_pres = self.presence_dist(
                    h_l, [bs], pri_wt=z_what, t=t, sample=True
                ).sample()
                z_where = self.transformation_dist(
                    h_l, [bs], pri_wt=z_what, sample=True
                ).sample()
            else:
                raise NotImplementedError
        else:
        ...
        new_state = GenState(
            h_l=h_l, h_c=h_c, z_pres=z_pres, z_what=z_what, z_where=z_where
        )
    ...
    def init_gen_state(self, init_h, init_z, bs):
        ...
        if init_z == None:
            ...
            z_what = torch.zeros(*bs, self.pts_per_strk, 2, device=self.device)
        else:
            z_pres, z_what, z_where = init_z

        state = GenState(
            h_l=h_l, h_c=h_c, z_pres=z_pres, z_where=z_where, z_what=z_what
        )
        return state
    ...
    def get_sample_curve(self, latents, uni_out_dim: bool = False, sample_res=50):
        ...
        sample_curves = self.decoder.get_sample_curve(trans_what, n_steps=n_steps)
        sample_curves = rearrange(sample_curves, "shp strk xy pts -> shp strk pts xy")
        ...
        for i, smpl in enumerate(sample_curves):
            ...
            uni_pts = (
                torch.unique(smpl[:keep_strks].reshape(keep_strks * n_steps, 2), dim=0)
                / smpl_res
            )
...
class SampleCurveDist(torch.distributions.Distribution):
    def __init__(self, cond_sample_curve, norm_std=0.1):
...
class SampleCurveDistWithAffine(torch.distributions.Distribution):
    def __init__(self, cond_sample_curve, norm_std=0.1):
        """Different from AffineSampleCurveDist, this performs transforms at log_prob
        eval time.

        And apply the transforms to the variables being
        ...
        self.cond_loc = cond_sample_curve.view(
            self.c_ptcs * self.c_bs, 1, self.c_n_pts, 2
        )
    ...
    def log_prob(self, sample_curve):
        ...
        for z_where in self.affines:
            ...
            comp = Independent(
                Normal(loc=trans_cond, scale=self.cond_std), reinterpreted_batch_ndims=1
            )
...
class AffineSampleCurveDist(torch.distributions.Distribution):
    def __init__(self, cond_sample_curve, norm_std=0.1):
        ...
        cond_sample_curve = cond_sample_curve.view(ptcs * bs, 1, ptns, 2).repeat(
            1, n_afn, 1, 1
        )
        ...
        cond_sample_curve = util.transform_z_what(
            z_what=cond_sample_curve, z_where=affines, z_where_type="7"
        )
...
class Guide(template.Guide):
    def __init__(
        ...
        self.internal_decoder = GenerativeModel(
            ...
            input_dependent_param=input_dependent_param,
            prior_dist=prior_dist,
    ...
    def forward(self, imgs, num_particles=1):  # , writer=None):
        ...
        (
            ...
            residual,
            strk_img,
        ) = self.initialize_state(imgs, ptcs)
        for t in range(self.max_strks):
            ...
            if self.intr_ll is None:
                result = self.inference_step(
                    p_state=state, imgs=imgs, canvas=canvas, residual=residual, t=t
                )
            else:
                # only pass in the most updated canvas
                result = self.inference_step(
                    state, imgs, canvas[:, :, t], residual, t=t
                )

            state = result["state"]
            assert (
                state.z_pres.shape == torch.Size([ptcs, bs, 1])
                and state.z_what.shape == torch.Size([ptcs, bs, self.pts_per_strk, 2])
                and state.z_where.shape == torch.Size([ptcs, bs, self.z_where_dim])
            )
            ...
            assert (
                result["z_pres_pms"].shape == torch.Size([ptcs, bs, 1])
                and result["z_what_pms"].shape
                == torch.Size([ptcs, bs, self.pts_per_strk, 2, 2])
            ...
            assert (
                result["z_pres_lprb"].shape == torch.Size([ptcs, bs, 1])
                and result["z_what_lprb"].shape == torch.Size([ptcs, bs, 1])
                and result["z_where_lprb"].shape == torch.Size([ptcs, bs, 1])
            )
            ...
            if self.use_canvas:
                ...
                self.internal_decoder.sgl_strk_tanh_slope = sgl_strk_tanh_slope[
                    :, :, t : t + 1
                ]
                ...
                if self.intr_ll is None:
                    ...
                    if self.linear_sum:
                        ...
                        if self.detach_canvas_so_far and t + 1 != self.max_strks:
                    ...
                    canvas_so_far = canvas[:, :, t : t + 1] + canvas_step.unsqueeze(2)
            ...
            if self.prior_dist == "Sequential":
                ...
                if self.dependent_prior:
                    ...
                    if self.prior_dependency == "wr|wt":
                        ...
                        pri_wr = None
                        if self.condition_by_img:
                            self.internal_decoder.sigma = sigmas[
                                :, :, t : t + 1
                            ].clone()
                            self.internal_decoder.sgl_strk_tanh_slope = (
                                sgl_strk_tanh_slope[:, :, t : t + 1]
                            ...
                            if self.feature_extractor_sharing:
                                ...
                                glmps_em = self.trans_img_feature_extractor(glmps).view(
                                    *shp, -1
                                )  # .detach()
                ...
                z_pres_prir[:, :, t] = (
                    self.internal_decoder.presence_dist(
                        h_l, [*shp], pri_wt, t=t
                    ).log_prob(z_pres_smpl[:, :, t].clone())
                    * mask_prev[:, :, t].clone()
                )
                z_where_prir[:, :, t] = (
                    self.internal_decoder.transformation_dist(
                        h_l, [*shp], pri_wt
                    ).log_prob(z_where_smpl[:, :, t].clone())
                    * z_pres_smpl[:, :, t].clone()
                )
                z_what_prir[:, :, t] = (
                    self.internal_decoder.control_points_dist(
                        h_c.clone(),
                        [*shp],
                        pri_wr,
        ...
        data = GuideReturn(
            z_smpl=ZSample(
                z_pres=z_pres_smpl, z_what=z_what_smpl, z_where=z_where_smpl
            ),
            z_pms=ZLogProb(z_pres=z_pres_pms, z_where=z_where_pms, z_what=z_what_pms),
            z_lprb=ZLogProb(
                z_pres=z_pres_lprb, z_what=z_what_lprb, z_where=z_where_lprb
            ),
            baseline_value=baseline_value,
            mask_prev=mask_prev,
            decoder_param=DecoderParam(
                sigma=sigmas, slope=(sgl_strk_tanh_slope, add_strk_tanh_slope)
            ),
            canvas=canvas,
            residual=residual,
            z_prior=ZLogProb(
                z_pres=z_pres_prir, z_what=z_what_prir, z_where=z_where_prir
            ),
    ...
    def inference_step(self, p_state, imgs, canvas, residual, t):
        """Given previous (initial) state and input image, predict the current step
        latent distribution.
        ...
        img_embed, canvas_embed, residual_embed, rsd_ratio = self.get_img_features(
            imgs, canvas, residual
        )

        # Predict z_pres, z_where from target and canvas
        pr_wr_mlp_in, h_l = self.get_pr_wr_mlp_in(
            img_embed, canvas_embed, residual_embed, rsd_ratio, p_state
        )
        ...
        if not self.residual_no_target:
            trans_imgs = util.spatial_transform(
                imgs.view(prod(shp), *img_dim),
                util.get_affine_matrix_from_param(
                    z_where.view(prod(shp), -1), z_where_type=self.z_where_type
                ),
            ).view(*shp, *img_dim)
        else:
            trans_imgs = None
        ...
        wt_mlp_in, h_c = self.get_wt_mlp_in(
            trans_imgs, trans_rsd, canvas_embed, p_state
        )
        ...
        out = {
            ...
            "z_pres_pms": z_pres_p,
            # [bs, pts_per_strk, 2, 2]
            "z_what_pms": torch.cat(
                (z_what_pms[0].unsqueeze(-1), z_what_pms[1].unsqueeze(-1)), dim=-1
            ),
            # [bs, z_where_dim, 2]
            "z_where_pms": torch.cat(
                (z_where_pms[0].unsqueeze(-1), z_where_pms[1].unsqueeze(-1)), dim=-1
            ),
    ...
    def get_z_l(self, pr_wr_mlp_in, p_state, rsd_ratio=None):
        ...
        if self.sep_where_pres_net:
            add_slope = self.glb_renderer_param_mlp(wr_mlp_in.view(prod(shp), -1))
        else:
            add_slope = self.glb_renderer_param_mlp(pr_wr_mlp_in.view(prod(shp), -1))
        # sigma = sigma.view(*shp, -1)
        # strk_slope = strk_slope.view(*shp, -1)
        add_slope = add_slope.view(*shp, -1)

        z_pres, z_where, z_pres_lprb, z_where_lprb = self.sample_pr_wr(
            p_state, z_pres_p, z_where_loc, z_where_scale, z_where_cor
        )
    ...
    def get_z_c(self, zwhat_mlp_in, p_state, z_pres, z_where):
        ...
        z_what_loc, z_what_std, z_what_cor = self.wt_mlp(
            zwhat_mlp_in.view(prod(shp), -1)
        )

        sigma, strk_slope = self.loc_renderer_param_mlp(
            torch.cat(
                [zwhat_mlp_in.view(prod(shp), -1), z_where.view(prod(shp), -1)], dim=1
            )
        )
        sigma = sigma.view(*shp, -1)
        strk_slope = strk_slope.view(*shp, -1)
        ...
        z_what, z_what_lprb = self.sample_wt(z_what_loc, z_what_std, z_what_cor, z_pres)
    ...
    def initialize_state(self, imgs, ptcs):
        ...
        if self.sep_where_pres_net:
            h_l = (
                torch.zeros(ptcs, bs, self.pr_wr_rnn_hid_dim, device=imgs.device),
                torch.zeros(ptcs, bs, self.pr_wr_rnn_hid_dim, device=imgs.device),
            )
        ...
        z_what_smpl = torch.zeros(
            ptcs, bs, self.max_strks, self.pts_per_strk, 2, device=imgs.device
        )
        z_where_smpl = torch.ones(
            ptcs, bs, self.max_strks, self.z_where_dim, device=imgs.device
        )
        # z distribution parameters for each step
        z_pres_pms = torch.ones(ptcs, bs, self.max_strks, device=imgs.device)
        z_what_pms = torch.zeros(
            ptcs, bs, self.max_strks, self.pts_per_strk, 2, 2, device=imgs.device
        )
        z_where_pms = torch.ones(
            ptcs, bs, self.max_strks, self.z_where_dim, 2, device=imgs.device
        )
        ...
        if self.prior_dist == "Sequential":
            z_pres_prir = torch.zeros(ptcs, bs, self.max_strks, device=imgs.device)
            z_what_prir = torch.zeros(ptcs, bs, self.max_strks, device=imgs.device)
            z_where_prir = torch.zeros(ptcs, bs, self.max_strks, device=imgs.device)
        elif self.prior_dist == "Independent":
            z_pres_prir, z_what_prir, z_where_prir = [None] * 3

        # baseline_value
        baseline_value = torch.zeros(ptcs, bs, self.max_strks, device=imgs.device)
        ...
        """Signal mask for each step At time t, `mask_prev` stroes whether
        prev.z_pres==0, `mask_curr` stores whether current.z_pres==0 after an
        `inference_step`.
        ...
        if self.linear_sum:
            strk_img = torch.zeros(
                ptcs, bs, self.max_strks, *self.img_dim, device=imgs.device
            )
        if self.use_canvas:
            if self.intr_ll:
                canvas = torch.zeros(
                    ptcs, bs, self.max_strks + 1, *self.img_dim, device=imgs.device
                )
            else:
        ...
        if self.sep_where_pres_net:
            h_prs = torch.zeros(
                ptcs, bs, self.max_strks, self.pr_wr_rnn_hid_dim, device=imgs.device
            )
            h_wrs = torch.zeros(
                ptcs, bs, self.max_strks, self.pr_wr_rnn_hid_dim, device=imgs.device
            )
    ...
    def character_conditioned_sampling(self, cond_img, n_samples=7):
        """Only useful when it has an internal render, o/w it returns None and one
        should use the param returned from `forward` to draw new samples for the
        generative model to render the image.
