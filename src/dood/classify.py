...
# import plot
from dood import losses, plot, util
...
def fine_tune(args, model, tst_loader, writer, continue_training=True):
    ...
    if Path(clf_checkpoint_path).exists() and continue_training:
        ...
        _, optimizer, scheduler, stats, _, trn_args = util.load_checkpoint(
            path=base_checkpoint_path, device=device, init_data_loader=False
        )
        stats.tst_elbos.clear()
    ...
    while ft_ite_so_far < finetune_ite:
        for sup_img, qry_img, _ in tst_loader:
            ...
            for n, l in zip(loss_tuple._fields, loss_tuple):
                writer.add_scalar(
                    "OneShot clf finetuning/" + n, l.detach().mean(), ft_ite_so_far
                )
            stats.tst_elbos.append(loss)

            if ft_ite_so_far % 50 == 0:
                util.logging.info(
                    f"Iteration {ft_ite_so_far} |" + f"Loss = {stats.tst_elbos[-1]:.3f}"
                )
            # save
...
def averaged_score(log_probs, log_score):
    ...
    # score [qs, ss]
    ave_log_score = logsumexp(
        log_score.detach().numpy(), axis=-1, b=weights.detach().numpy()
    )
    ...
    for eps, (sup_img, qry_img, label) in tqdm(enumerate(tst_loader)):
        ...
        if optimize:
            ...
            for direction in directions:
                ...
                for run in range(run_per_eps):
                    ...
                    optim = torch.optim.Adam(
                        [{"params": cm.optim_parameters(), "lr": 1e-2}]
                    )
                    # Optimize
                    cm.train()
                    writer.add_image(
                        f"One shot classification/Run{eps} Query",
                    ...
                    for ite in tqdm(range(100)):
                        ...
                        if direction == "s|q":
                            in_img = sup_img
                        log_score, recons, qrys, untrans_optim_z_plot = cm(
                            in_img,
                        ...
                        # Log
                        writer.add_scalar(
                            f"One shot classification/Run{eps} scores", m_score, ite
                        )
                        if ite % 100 == 0:
                            disp = make_grid(display_trans(recons[:, 0]), nrow=20)
                            writer.add_image(
                                f"One shot classification/eps{eps} recons0", disp, ite
                            )
                            disp = make_grid(display_trans(recons[:, 1]), nrow=20)
                            writer.add_image(
                                f"One shot classification/eps{eps} recons1", disp, ite
                            )
                            disp = make_grid(display_trans(qrys[:, 0]), nrow=20)
                            writer.add_image(
                                f"One shot classification/eps{eps} qry_mtrx0", disp, ite
                            )
                            disp = make_grid(display_trans(qrys[:, 1]), nrow=20)
                            writer.add_image(
                                f"One shot classification/eps{eps} qry_mtrx1", disp, ite
                            )
                            if render_untrans:
                                disp = make_grid(
                                    display_trans(untrans_optim_z_plot[:, 0]), nrow=20
                                )
                                writer.add_image(
                                    f"One shot classification/eps{eps} untrans_optim_z0",
                                    disp,
                                    ite,
                                )
                                disp = make_grid(
                                    display_trans(untrans_optim_z_plot[:, 1]), nrow=20
                                )
                                writer.add_image(
                                    f"One shot classification/eps{eps} untrans_optim_z1",
                                    disp,
                                    ite,
                                )
                    ...
                    if direction == "q|s":
                        ...
                        print(
                            f"##### eps {eps} run {run}/{run_per_eps-1} accuracy (1-way):",
                            one_w_accuracy,
                        )
            ...
            if two_way_clf:
                ...
                img_lld = torch.logsumexp(sup_elbos, dim=1)[None,] - torch.log(
                    torch.tensor(ptcs)
                )
                # scores[0]: [qs, ss]
...
class ClassifyModel(nn.Module):
    def __init__(
        ...
        if model_type == "Sequential":
            ...
            z_wt = util.transform_z_what(
                z_wt.view(ps * ss * 2, strks, pts, 2),
            ...
            if direction == "q|s":
                ...
                optm_sigma, optm_strk_tanh, optm_add_tanh = (
                    ...
                    .transpose(1, 2)
                    .repeat(qs, 1, 1, 1),
                    dec_param[1][0][None, :, :20]
                    .clone()
                    .transpose(1, 2)
                    .repeat(qs, 1, 1, 1),
                    dec_param[1][1][None, :, :20, 0]
                    .clone()
                    .transpose(1, 2)
                    .repeat(qs, 1, 1),
                )
            ...
            self.affines_param_ = torch.nn.Parameter(
                torch.zeros([qs, ss, ps, 7], device=z_wt.device), requires_grad=True
            )
            self.gen.sigma = optm_sigma.view(qs * ss, ps, strks).cuda()
            self.gen.sgl_strk_tanh_slope = optm_strk_tanh.view(
                qs * ss, ps, strks
            ).cuda()
    ...
    def get_affines(self):
        shift = util.constrain_parameter(self.affines_param_[..., :2], -0.2, 0.2)
        scale = util.constrain_parameter(self.affines_param_[..., 2:4], 0.6, 1.4)
        rot = util.constrain_parameter(
            self.affines_param_[..., 4:5], -0.25 * math.pi, 0.25 * math.pi
        )
        shear = util.constrain_parameter(
            self.affines_param_[..., 5:7], -0.3 * math.pi, 0.3 * math.pi
        )
    ...
    def get_cond_token_dist(self):
        if self.model_type == "Sequential":
            ...
            token_dist = Independent(
                Normal(self.cond_wt, self.cond_wt_std), reinterpreted_batch_ndims=2
            )
            ...
            #                     ).view(qs, ss, ps, strks, pts, 2)
            token_dist = Independent(
                Normal(self.cond_wt, self.cond_wt_std), reinterpreted_batch_ndims=1
            )
        return token_dist
    ...
    def forward(self, qry_img, render_untrans=False, ite=None, writer=None):
        ...
        if self.model_type == "Sequential":
            ...
            self.optm_wt = torch.nn.Parameter(
                torch.nan_to_num(self.optm_wt), requires_grad=True
            )
            # Compute the token model log_prob; averaged by the number of steps
            strks_per_samp = self.cond_pr.sum(-1)  # [qs, ss, ps]
            token_dist = self.get_cond_token_dist()
            ...
            af_mtrx = util.get_affine_matrix_from_param(
                affine_params.view(qs * ss * ps, -1), "4_rotate"
            )
            ...
            recons = torch.clip(recons, min=0.0, max=1.0)
            qry_img = torch.clip(qry_img, min=0.0, max=1.0)
            img_dist = self.gen.img_dist(canvas=recons)
            recons = recons.cpu()
            image_score = img_dist.log_prob(qry_img).view(qs, ss, ps)
...
def score():
    with torch.no_grad():
        ...
        for run, (sup_img, qry_img, label) in tqdm(enumerate(tst_loader)):
            ...
            sup_latents = util.get_top_k_latents(guide, gen, sup_img, ptcs=3, k=1)
            ...
            # norm_std = 0.05
            norm_std = 0.05
            sup_crv = gen.get_sample_curve(
                sup_latents, uni_out_dim=True, sample_res=sample_res
            )
            ...
            qry_crv = gen.get_sample_curve(
                qry_latents, uni_out_dim=True, sample_res=sample_res
            )
            ...
            writer.add_image(
                "One shot classification/query img",
                make_grid(display_transform(qry_img), nrow=10),
                run + 1,
            )
            writer.add_image(
                "One shot classification/support img",
                make_grid(display_transform(sup_img), nrow=10),
                run + 1,
            )
