...
def to_logit(x):
    ...
    renorm = renorm.clamp(min=eta, max=(1.0 - eta))
    return renorm.log() - (1.0 - renorm).log()
...
def get_design_matrix(points, order=3):
    ...
    feat = [
        comb(order, k) * t ** (order - k) * (1 - t) ** (k) for k in range(order + 1)
    ]
...
def train(epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        output = model(data)
        loss = (
            F.l1_loss(gauss(output), gauss(data), reduction="sum") / gauss(output).sum()
        )
...
def debug_one_datapoint(img, steps=100):
    ...
    for i in range(steps):
        ...
        loss = (
            F.l1_loss(output, img, reduction="sum")
            + 20 * ((control_points - 0.5) ** 2).sum()
        )
    ...
    save_image(
        make_grid(torch.cat(recons), nrow=10, pad_value=255),
        "../results/debug_recon/recons.png",
    )
...
def test():
    ...
    with torch.no_grad():
        ...
        n = min(data.size(0), 8)
        comparison = torch.cat([data[:n], output.view(16, 1, 28, 28)[:n]])
        save_image(
