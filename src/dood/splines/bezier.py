...
class Bezier(torch.nn.Module):
    """Takes in tensors shaped (batches, curves, control_points, x/y) and returns
    tensors representing greyscale images shaped (batches, height, width)"""
    ...
    def __init__(self, res=512, steps=128, method="base", debug=False):
        ...
        # C.shape: [res, res]
        C, D = torch.meshgrid(
            torch.Tensor(range(self.res)), torch.Tensor(range(self.res))
        )
        # c.shape [1, 1, res, res]
        self.c = torch.nn.Parameter(
            C.unsqueeze(0).unsqueeze(0) / res, requires_grad=False
        )
        self.d = torch.nn.Parameter(
            D.unsqueeze(0).unsqueeze(0) / res, requires_grad=False
        )
    ...
    def forward(self, control_points, sigma: float, keep_strk_dim: bool):
        ...
        if keep_strk_dim:
            ...
            curve = rearrange(
                self.sample_curve(control_points, steps),
    ...
    def _raster_base(self, curve, sigma=5e-2):
        ...
        c = torch.transpose(self.c.expand(batch_size, steps, self.res, self.res), 1, 3)
        d = torch.transpose(self.d.expand(batch_size, steps, self.res, self.res), 1, 3)
    ...
    def _raster_bounded(self, curve, sigma=1e-2 * 2):
        ...
        if torch.is_tensor(sigma):
            ...
            xmax = torch.clamp(
                (self.res * (xmax + 3 * sig_max)).ceil(), 0, self.res
            ).int()
            ymax = torch.clamp(
                (self.res * (ymax + 3 * sig_max)).ceil(), 0, self.res
            ).int()
            xmin = torch.clamp(
                (self.res * (xmin - 3 * sig_max)).floor(), 0, self.res
            ).int()
            ymin = torch.clamp(
                (self.res * (ymin - 3 * sig_max)).floor(), 0, self.res
            ).int()
        else:
            xmax = torch.clamp(
                (self.res * (xmax + 3 * sigma)).ceil(), 0, self.res
            ).int()
            ymax = torch.clamp(
                (self.res * (ymax + 3 * sigma)).ceil(), 0, self.res
            ).int()
            xmin = torch.clamp(
                (self.res * (xmin - 3 * sigma)).floor(), 0, self.res
            ).int()
            ymin = torch.clamp(
                (self.res * (ymin - 3 * sigma)).floor(), 0, self.res
            ).int()
    ...
    def _raster_bounded_tight(self, curve, sigma=1e-2):
        ...
        print(theta)
        R = torch.Tensor([[theta.cos(), theta.sin()], [-theta.sin(), theta.cos()]]).to(
            curve.device
        )
        ...
        y = curve[1]
        xmax, ymax = [
            (self.res * (i.max() + 3 * sigma)).ceil().int().item() for i in (x, y)
        ]
        xmin, ymin = [
            (self.res * (i.min() - 3 * sigma)).floor().int().item() for i in (x, y)
        ]
        w = xmax - xmin
        h = ymax - ymin
        print(xmin, xmax)
        ...
        raster[self.res + xmin : self.res + xmax, self.res + ymin : self.res + ymax] = (
            raster_
        )
