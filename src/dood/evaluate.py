"""Load trained models and evaluate them via classification and marginal likelihood."""
...
from dood import losses, plot, train, util
from dood.models.ssp import (
    AffineSampleCurveDist,
    SampleCurveDist,
    SampleCurveDistWithAffine,
)

# from models.mws.handwritten_characters.train import get_log_p_and_kl
from dood.models.template import ZLogProb, ZSample
from dood.plot import display_transform
    ...
    with torch.no_grad():
        ...
        for imgs, target in test_loader:
            ...
            if args.model_type == "Base":
                ...
                loss_tuple = losses.get_loss_vae(generative_model, guide, imgs, k)
        ...
        if not only_reconstruction:
            if args.model_type == "MWS":
                ...
                for n, l in zip(names, loss_tuple):
                    writer.add_scalar(
                        f"{dataset_name}/Test curves/" + n, l.detach().mean(), epoch
                    )
            else:
                for n, l in zip(loss_tuple._fields, loss_tuple):
                    ...
                    if dataset_name is not None:
                        writer.add_scalar(
                            f"{dataset_name}/Test curves/" + n, l, ite_so_far
                        )
                    else:
        ...
        if args.model_type == "MWS":
            util.logging.info(
                f"Epoch {epoch} Test loss | Loss = {loss_tuple[0].detach().mean():.3f}"
            )
        else:
            util.logging.info(
                f"Epoch {epoch} Test loss | Loss = {loss_tuple.overall_loss:.3f}"
            )
        ...
        if log_to_file:
            log_file_name = "results/metrics/eval_mll.csv"
...
def stroke_mll_plot(model, val_loader, args, writer, epoch):
    """Make the stroke vs mll plot for images of 1s and 7s and save it both in a file
    and log it on tensorboard."""
    n_strks, mlls, labels = [], [], []
    gen, guide = model
    ...
    # plot
    plot_data = pd.DataFrame({"Num_strokes": n_strks, "ELBO": mlls, "Label": labels})
...
def classification_evaluation(
    ...
    """
    ite_so_far = len(stats.trn_losses)
    guide_classifier, dataloaders, optimizer, clf_stats = util.init_classification_nets(
        guide, args, dataset, batch_size, trned_ite=ite_so_far
    )
    ...
    while iteration < num_iterations:
        ...
        for zs, target in train_loader:
            ...
            if iteration % log_interval == 0:
                util.logging.info(
                    "Train iteration: {}/{}\t clf_accuracy: {:.6f}".format(
                        iteration, num_iterations, clf_accuracy
                    )
                )
            writer.add_scalar(
                f"{dataset_name}/Train/Classification_accuracy", clf_accuracy, iteration
            )
        ...
        with torch.no_grad():
            ...
            writer.add_scalar(
                f"{dataset}/Test/Classification_accuracy", guide_clf_loss, epoch
            )
            clf_stats.tst_accuracy.append(guide_clf_loss.item())

    if log_to_file:
        log_file_name = "results/metrics/eval_clf.csv"
        if os.path.exists(log_file_name):
            print("File exist.")
            eval_df = pd.read_csv(log_file_name, header=0, index_col=0)
            print(eval_df)
...
def unconditioned_generation(model, args, writer, in_img, stats, max_strks=None):
    ...
    if max_strks == None:
        ...
        writer.add_image(f"Unconditioned Generation/in_img", in_img_grid, ite_so_far)
    ...
    with torch.no_grad():
        ...
        if max_strks == None:
            ...
            writer.add_image(
                f"Unconditioned Generation/out_img", gen_img_grid, ite_so_far
            )
...
def one_shot_classification(
    ...
    # Finetune the model
    if do_fine_tune:
        tst_loader = util.init_ontshot_clf_data_loader(res=args.img_res, shuffle=True)
        model = fine_tune(args, model, tst_loader, writer)

    # Parse again
    tst_loader = util.init_ontshot_clf_data_loader(res=args.img_res, shuffle=False)
    # latents, dec_params, gen_probs = parse(args, model, tst_loader, writer,
...
def num_strokes_plot(model, stats, args, writer, max_strks=10):
    ...
    for ds in ["KMNIST", "EMNIST"]:
        datasets.remove(ds)  # simplify dsets

    # loop through all dataset
    for ds_name in datasets:
        _, test_loader, _, _ = util.init_dataloader(
            args.img_res, ds_name, batch_size=args.batch_size
        )
        ...
        for i, (imgs, _) in enumerate(tqdm(test_loader)):
            ...
            if i == 0:
                if args.model_type == "Sequential":
                    ...
                    gen.sgl_strk_tanh_slope = out.decoder_param.slope[0]
                    if guide.linear_sum:
                        gen.add_strk_tanh_slope = out.decoder_param.slope[1][:, :, 0]
                    else:
                        gen.add_strk_tanh_slope = out.decoder_param.slope[1][:, :, -1]
                plot.plot_cum_recon(
                    args,
                    imgs,
                    gen,
    ...
    df = pd.concat(
        [
            pd.DataFrame({"dataset": name, "number of strokes": n_strks})
    ...
    fig.savefig(f"results/figures/z_pr_generalize_{args.save_model_name}.pdf")
...
def auto_complete(model, args, writer, partial_img, stats):
    ...
    writer.add_image(f"Partial completion/in_img", in_img_grid, ite_so_far)
    ...
    # top parse
    z_smpl, hs, dec_param, recs, _, _ = util.get_top_latents_hiddens(
        guide, gen, partial_img, ptcs=1
    )
    last_hs, last_z_sample = util.get_last_vars(hs, z_smpl)
    ...
    partial_img = (
        dec_param.slope[1][0:1, :, 0].squeeze(0)[:, None, None, None]
        *
    ...
    writer.add_image(
        f"Partial completion/partial image recons", rec_img_grid, ite_so_far
    )
    # - log the completion results
    gen_imgs = gen_out.canvas
    gen_imgs_l = display_transform(gen_imgs.squeeze(0))
    gen_img_grid = 1 - make_grid(gen_imgs_l, nrow=NROW)
    writer.add_image(f"Partial completion/out_img", gen_img_grid, ite_so_far)
    # save
    gen_imgs_wrap = (
        gen_imgs_l.view(4, 8, 1, 64, 64).transpose(1, 2).reshape(4, 1, 8 * 64, 64)
    )
    cat = 1 - torch.cat([par_img_set, gen_imgs_wrap], dim=2)
    for i in range(4):
        save_imgs_dir = (
            f"/om/user/ycliang/{args.dataset}-par_com-{(7+int(args.seed))*4 + i}.pdf"
        )
...
def draw_samples(generative_model, guide, bs, dec_param, max_strks=None):
    with torch.no_grad():
        ...
        gen_out = generative_model.sample(
            bs=[bs],
...
def init_generator(args, model, in_img):
    ...
    with torch.no_grad():
        ...
        decoder_param = DecoderParam(
            sigma=decoder_param.sigma[:, :, 0:1]
            .median()
            .repeat(1, bs, guide.max_strks),
            slope=[
                decoder_param.slope[0][:, :, 0:1]
                .median()
                .repeat(1, bs, guide.max_strks),
                decoder_param.slope[1][:, :, 0:1]
                .median()
                .repeat(1, bs, guide.max_strks),
            ],
        )
...
def get_args_parser():
    ...
    parser.add_argument(
        "--ckpt_path", default=None, type=str, help="Path to checkpoint for evaluation"
    )

    parser.add_argument(
        "--save_model_name", default=None, type=str, help="name for ckpt dir"
    )
    parser.add_argument(
        "--tb_name", default=None, type=str, help="name for tensorboard log"
    )
...
if __name__ == "__main__":
    ...
    model, optimizer, scheduler, stats, _, trn_args = util.load_checkpoint(
        path=ckpt_path, device=device, init_data_loader=False
    )
    ...
    if plot_tsne:
        with torch.no_grad():
            ...
            for n in n_clusters:
                plot.plot_stroke_tsne(
                    trn_args,
                    ckpt_path=ckpt_path,
                    title=f"tsne",
                    save_dir=f"results/figures/tsne_{args.save_model_name}"
                    + f"_{coloring}{n}",
    ...
    if num_strokes_eval:
        with torch.no_grad():
            num_strokes_plot(model=model, stats=stats, args=trn_args, writer=writer)

    for dataset in marginal_likelihood_test_datasets:
        if recon_eval:
            train_loader, test_loader, _, _ = util.init_dataloader(
                res, dataset, batch_size=64, rot=False
            )
    ...
    if char_con_sample:
        train_loader, test_loader, _, _ = util.init_dataloader(
            res, trn_args.dataset, batch_size=32, rot=False
        )
    ...
    if clf_eval:
        for dataset in clf_test_datasets:
            ...
            classification_evaluation(
                ...
                dataset_name=dataset,
                batch_size=64,
                clf_trn_interations=10 if test_run else args.clf_trn_interations,
            )
    ...
    if uncon_sample or auto_complete_demo or compute_fid:
        num_to_sample = 64
        trn_loader, _, _, _ = util.init_dataloader(
            res, trn_args.dataset, batch_size=num_to_sample, rot=True
        )
        ...
        if uncon_sample:
            ...
            print(
                f"===> Done Unconditioned generation on with {args.save_model_name}\n"
            )
        if auto_complete_demo:
            ...
            if partial_img == None:
                ...
                partial_img = unconditioned_generation(
                    model=model,
                    args=trn_args,
                    writer=writer,
                    in_img=in_img[:num_to_sample],
