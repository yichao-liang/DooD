...
class MultiMnistDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        image = util.normalize_pixel_values(
            img=image.unsqueeze(0), method="maxnorm"
        ).squeeze(0)
    ...
    def get_all_numbers(self, num_digits):
        """Get all numbers of digits `num_digits` in their num_digits-digits form.

        E.g. num_digits=2 results in 00, 01, ... 99.
        """
        l = range(10)
...
def get_dataloader(
    ...
    dataset = MultiMnistDataset(
        root=root, device=device, keep_classes=keep_classes, transform=transform
    )
