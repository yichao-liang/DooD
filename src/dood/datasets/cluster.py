...
def get_lv_data_loader(
    ...
    for i, split in enumerate(["train", "test"]):
        ...
        if trans_z_what:
            lv_path = (
                f"./data/latent_dataset/{dataset_name}-by-{model_name}"
        ...
        try:
            if remake_data:
                raise FileNotFoundError  # remake dataset
    ...
    for imgs, ys in dataloader:
        ...
        if model_type == "MWS":
            ...
            if model_type != "VAE":
                ...
                z_what = z_what.reshape(bs, max_strks, -1) * z_pres.view(
                    bs, max_strks, -1
                )
                z_where = z_where.view(bs, max_strks, -1) * z_pres.view(
                    bs, max_strks, -1
                )
...
def make_medoid_dataset(dataset_path, lv_path, guide, dataloaders, re_infer=False):
    ...
    wr_medoids_data, wr_medoids, wr_clusters = get_medoids(
        z_wheres.view(prod(shp), z_where_dim).numpy(), z_press.numpy()
    )
    wt_medoids_data, wt_medoids, wt_clusters = get_medoids(
        z_whats.view(prod(shp), ndim).numpy(), z_press.numpy()
    )

    # [dataset_size, max_strks (eg. 4) * 10 (pts * 2)]
    wr_medoids_data = torch.tensor(wr_medoids_data).view(-1, max_strks * z_where_dim)
    wt_medoids_data = torch.tensor(wt_medoids_data).view(-1, max_strks * ndim)
    memoids_data = torch.concat([wr_medoids_data, wt_medoids_data], dim=-1)

    print("===> Done making the dataset dict")

    dataset_dict = {
        "memoids_data": memoids_data,
        "labels": labels,
