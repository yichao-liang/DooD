...
from dood.util import incremental_average
...
def get_loss_sequential(
    ...
    if args.constrain_z_pres_param and iteration < 10000:
        ...
        guide.constrain_z_pres_param_this_ite = False
    ...
    if (
        canvas is None and generative_model.input_dependent_param
        ...
        generative_model.sgl_strk_tanh_slope = guide_out.decoder_param.slope[0]
        generative_model.add_strk_tanh_slope = guide_out.decoder_param.slope[1][
            :, :, -1
        ]
        if args.render_at_the_end:
            canvas = None
    # multi by beta
    # log_post_ = {}
    ...
    if args.global_reinforce_signal:
        ...
        bl_target = torch.cat(
            [prob.flip(-1).cumsum(-1).flip(-1).unsqueeze(-1) for prob in log_post],
            dim=-1,
        ).sum(-1)
        bl_target -= torch.cat(
            [prob.flip(-1).cumsum(-1).flip(-1).unsqueeze(-1) for prob in log_prior],
            dim=-1,
        ).sum(-1)
    ...
    if args.update_reinforce_loss:
        ...
        signal_var = (
            ((reinforce_sgnl - signal_mean) * mask_prev) ** 2
        ).sum() / num_nonzero
        if iteration == 0:
        ...
        reinforce_sgnl = (reinforce_sgnl - guide.c) / max(1, torch.sqrt(guide.v))
    ...
    if writer is not None:
        with torch.no_grad():
            ...
            if train:
                ...
                writer.add_scalar(
                    f"{writer_tag}Train curves/reinforce variance",
                    reinforce_sgnl.detach().var(),
                    iteration,
                )
                # loss
                for n, log_prob in zip(log_post._fields, log_post):
                    writer.add_scalar(
                        f"{writer_tag}Train curves/log_posterior/" + n,
                        log_prob.detach().sum(-1).mean(),
                        iteration,
                    )
                for n, log_prob in zip(["z_pres", "z_where", "z_what"], log_prior):
                    writer.add_scalar(
                        f"{writer_tag}Train curves/log_prior/" + n,
                        log_prob.detach().sum(-1).mean(),
                        iteration,
                    )
                writer.add_scalar(
                    f"{writer_tag}Train curves/# of 1s in z_pres",
                    z_pres_smpls.sum(),
                    iteration,
                )
            ...
            if args.log_param and iteration % 50 == 0:
                ...
                if log_z_post_samples:
                    ...
                    if guide.z_where_type == "5":
                        ...
                        z_where_rot = (
                            z_where[:, :, :, 4].view(prod(shp), -1).numpy()[z_pres == 1]
                        )
                    else:
                        z_where_shift = (
                            z_where[:, :, :, 0:2]
                    ...
                    if z_pres.sum() > 0:
                        writer.add_histogram(
                            f"{writer_tag}Samples/z_where.scale",
                            z_where_scale,
                            iteration,
                        )
                        writer.add_histogram(
                            f"{writer_tag}Samples/z_where.shift",
                            z_where_shift,
                            iteration,
                        )
                        if guide.z_where_type in ["5", "4_rotate"]:
                            writer.add_histogram(
                                f"{writer_tag}Samples/z_where.rotate",
                                z_where_rot,
                                iteration,
                            )
                        writer.add_histogram(
                            f"{writer_tag}Samples/z_what", z_what, iteration
                        )
    if average_particle_score:
        ...
        log_likelihood = torch.logsumexp(log_likelihood, dim=0) - torch.log(
            torch.tensor(k)
        )
        log_prior_z = torch.logsumexp(log_prior_z, dim=0) - torch.log(torch.tensor(k))
        log_post_z = torch.logsumexp(log_post_z, dim=0) - torch.log(torch.tensor(k))
...
def get_loss_base(generative_model, guide, imgs, loss="elbo"):
    ...
    if torch.is_tensor(imgs):
        ...
        if loss == "l1":
            loss = F.l1_loss(recon_img, imgs, reduction="none").sum(
                (1, 2, 3)
            ) - recon_img.sum((1, 2, 3))
        elif loss == "elbo":
...
def get_loss_air(
    ...
    # (flip -> cumsum -> flip) so that it cumulate on to the left
    bl_target = torch.cat(
        [prob.flip(-1).cumsum(-1).flip(-1).unsqueeze(-1) for prob in log_post], dim=-1
    ).sum(-1)
    bl_target -= torch.cat(
        [prob.flip(-1).cumsum(-1).flip(-1).unsqueeze(-1) for prob in log_prior], dim=-1
    ).sum(-1)
    ...
    if writer != None:
        with torch.no_grad():
            ...
            if args.log_param and iteration % 50 == 0:
                ...
                writer.add_histogram(
                    "Parameters/z_pres", guide_out.z_pms.z_pres.detach(), iteration
                )
                writer.add_histogram(
                    "Parameters/z_where_posterior.loc.scale",
                    guide_out.z_pms.z_where.detach()[:, :, 0, 0],
                    iteration,
                )
                writer.add_histogram(
                    "Parameters/z_where_posterior.loc.shift",
                    guide_out.z_pms.z_where.detach()[
                        :,
                        :,
                        1:,
                        0,
                ...
                writer.add_histogram(
                    "Parameters/z_where_posterior.scale",
                    guide_out.z_pms.z_where.detach()[:, :, :, 1],
                    iteration,
                )
                writer.add_histogram(
                    "Parameters/z_what_posterior.loc",
                    guide_out.z_pms.z_what.detach()[:, :, :, 0],
                    iteration,
                )
                writer.add_histogram(
                    "Parameters/z_what_posterior.scale",
                    guide_out.z_pms.z_what.detach()[:, :, :, 1],
                    iteration,
                )
    ...
    loss = torch.logsumexp(loss, dim=0) - torch.log(torch.tensor(k))
    elbo = torch.logsumexp(elbo, dim=0) - torch.log(torch.tensor(k))
    log_likelihood = torch.logsumexp(log_likelihood, dim=0) - torch.log(torch.tensor(k))
    log_prior_z = torch.logsumexp(log_prior_z, dim=0) - torch.log(torch.tensor(k))
    log_post_z = torch.logsumexp(log_post_z, dim=0) - torch.log(torch.tensor(k))
...
def get_loss_vae(generative_model, guide, imgs, iteration=None, writer=None, k=1):
    """
    Args:
        loss (str): "nll": negative log likelihood, "l1": L1 loss, "elbo": -ELBO
    """
    guide_out = guide(imgs, k)
    z_smpl, (z_loc, z_std), guide_log_prob = (
        guide_out.z_smpl,
        guide_out.z_pms,
        guide_out.z_lprb,
    )
