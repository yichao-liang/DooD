...
def get_image_probs(
    ids, on_offs, start_point, primitives, rendering_params, num_rows, num_cols
):
    """
    Args:
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        if self.alphabet is None:
            lstm_input = torch.zeros(
                (*sample_shape, self.lstm_input_size), device=device
            ...
            lstm_input = torch.cat(
                [
                    torch.zeros(
                        (num_samples * num_batch, self.lstm_input_size - 50),
                        device=device,
                    ),
                    alphabet_expanded,
                ],
        ...
        for arc_id in range(self.num_arcs):
            ...
            if self.uniform_mixture:
                ...
                id_logits = util.logit_uniform_mixture(
                    logits[:, : self.num_primitives], p
                )
                on_off_logits = util.logit_uniform_mixture(
                    logits[:, self.num_primitives :], p
                )
            else:
        ...
        return torch.stack(
            [torch.stack(ids, dim=1), torch.stack(on_offs, dim=1)], dim=-1
        ).view(*sample_shape, *self.batch_shape, self.num_arcs, 2)
    ...
    def log_prob(self, ids_and_on_offs):
        ...
        for arc_id in range(self.num_arcs):
            ...
            id_dist = torch.distributions.Categorical(
                logits=id_logits.view(
                    *sample_shape, *self.batch_shape, self.num_primitives
                )
            )
            ...
            lstm_input = torch.cat(
                [
                    F.one_hot(
                        ids[..., arc_id].view(-1), num_classes=self.num_primitives
                    ).float(),
...
def get_thetas(device):
    ...
    for (
        ...
        scale_matrix = get_scale_matrix(
            scale_horizontal_squeeze, scale_vertical_squeeze
        )
        translate_matrix = get_translation_matrix(translate_x, translate_y)
        result.append(
            compose(shear_matrix, rotate_matrix, scale_matrix, translate_matrix)
        )
...
class AffineLikelihood(torch.distributions.Distribution):
    def __init__(self, cond, thetas=None):
        ...
        self.grid = F.affine_grid(
            self.thetas, self.cond_expanded.size(), align_corners=True
        )

        # num_thetas, batch_size, 28, 28
        self.cond_transformed = F.grid_sample(
            self.cond_expanded, self.grid, align_corners=True
        )
        self.dist = torch.distributions.Independent(
            torch.distributions.Bernoulli(probs=self.cond_transformed),
    ...
    def log_prob(self, obs):
        ...
        """
        return torch.logsumexp(self.dist.log_prob(obs), dim=0) - math.log(
            self.num_thetas
        )
...
class ClassificationLikelihood(torch.distributions.Distribution):
    ...
    def log_prob_with_id(self, obs, obs_id, get_accuracy=False):
        ...
        #                     .reshape(n_obs, n_classes)
        log_likelihood = self.class_conditional_model.forward(obs, obs_id).reshape(
            n_obs, n_classes
        )
        ...
        if get_accuracy:
            ...
            best_class_probs = self.class_probs.gather(
                1, best_class_indices.reshape(-1, 1)
            )[:, 0]
...
class GenerativeModel(nn.Module):
    def __init__(
        ...
        self._likelihood.pre_dxdy = nn.Parameter(
            torch.rand((num_primitives, 2)) * (pre_dxdy_max - pre_dxdy_min)
            + pre_dxdy_min
        )
        ...
        theta_init = (
            torch.rand((num_primitives,)) * (init_theta_max - init_theta_min)
            + init_theta_min
        )
        if self.big_arcs:
            ...
            p = 0.01 + 0.98 * p
            assert p.min() > 0 and p.max() < 1
            self._likelihood.pre_theta = nn.Parameter(
                (p.log() - (1 - p).log()) / self.theta_factor
            )
        ...
        if self.pixelcnn_likelihood:
            ...
            if likelihood == "classify" or likelihood.startswith("classify:"):
                self.classifier = torch.load(
                    "classifier.pt", map_location=torch.device("cpu")
                )
            ...
            for p in [
                ...
                *self.class_conditional_model.parameters(),
            ]:
        ...
        assert (
            len(set(self._prior.parameters()) & set(self._likelihood.parameters())) == 0
        )
    ...
    def get_primitives(self):
        ...
        if self.big_arcs:
            ...
            theta = (
                torch.remainder(self._likelihood.pre_theta * math.pi, 2 * math.pi)
                - math.pi
            )
        else:
            theta = (
        ...
        return torch.cat(
            [dxdy, theta.unsqueeze(-1), sharpness.unsqueeze(-1), width.unsqueeze(-1)],
            dim=1,
        )
    ...
    def get_log_prob(self, latent, obs, obs_id, get_accuracy=False):
        ...
        obs_dist = self.get_obs_dist(
            latent.view(num_particles * batch_size, num_arcs, 2)
        )
        if hasattr(obs_dist, "log_prob_with_id"):
            obs_log_prob, accuracy = obs_dist.log_prob_with_id(
                ...
                obs_id[None]
                .expand(num_particles, batch_size)
    ...
    def sample_latent_and_obs(self, alphabet=None, num_samples=1):
        ...
        if self.use_alphabet:
            ...
            obs_dist = self.get_obs_dist(
                latent.view(num_samples * batch_size, self.num_arcs, 2)
            )
            obs = obs_dist.sample().view(
                num_samples, batch_size, self.num_rows, self.num_cols
            )
        else:
...
class InferenceNetworkIdsAndOnOffsDistribution(torch.distributions.Distribution):
    ...
    def sample(self, sample_shape=torch.Size()):
        ...
        if self.alphabet is None:
            ...
            alphabet_expanded = (
                self.alphabet[None]
                .expand(num_samples, self.batch_size, 50)
                .reshape(-1, 50)
            )
            obs_embedding_expanded = (
                self.obs_embedding[None]
            ...
            h = torch.zeros(
                (num_samples * self.batch_size, self.lstm_hidden_size), device=device
            )
            c = torch.zeros(
                (num_samples * self.batch_size, self.lstm_hidden_size), device=device
            )
        ...
        for arc_id in range(self.num_arcs):
            ...
            if arc_id == 0:
                on_off = (
                    torch.zeros((*sample_shape, self.batch_size), device=device)
                    .long()
                    .view(-1)
                )
            else:
        ...
        return torch.stack(
            [torch.stack(ids, dim=1), torch.stack(on_offs, dim=1)], dim=-1
        ).view(*sample_shape, self.batch_size, self.num_arcs, 2)
    ...
    def log_prob(self, ids_and_on_offs):
        ...
        if self.alphabet is None:
            ...
            obs_embedding_expanded = (
                self.obs_embedding[None]
                .expand(
                    int(np.prod(sample_shape)), self.batch_size, self.obs_embedding_dim
                )
                .reshape(-1, self.obs_embedding_dim)
            )
            lstm_input[:, -self.obs_embedding_dim :] = obs_embedding_expanded
        ...
        for arc_id in range(self.num_arcs):
            ...
            id_dist = torch.distributions.Categorical(
                logits=id_logits.view(
                    *sample_shape, self.batch_size, self.num_primitives
                )
            )
            on_off_dist = torch.distributions.Categorical(
...
def get_args_parser():
    ...
    parser.add_argument(
        "--small-dataset", action="store_true", help="use a small dataset"
    )
    ...
    # train
    parser.add_argument(
        "--algorithm", default="mws", choices=["mws", "rws", "vimco"], help=" "
    )
...
def distance_to_arc(points, arcs):
    ...
    return small_arc * (
        inside_sector * inside_sector_distance
        + outside_sector * outside_sector_distance
    ) + large_arc * (
        inside_sector * outside_sector_distance
        + outside_sector * inside_sector_distance
    )
...
def get_logits(arcs, on_offs, rendering_params, num_rows, num_cols):
    ...
    result = (
        torch.max(
            get_value(points, arcs, scale, bias) + on_offs.float().log().unsqueeze(-2),
            dim=-1,
        )[0]
    ...
    result = torch.logsumexp(
        torch.stack([result, torch.ones(result.shape, device=result.device) * epsilon]),
        dim=0,
    )
...
def init_optimizer(generative_model, inference_network, prior_lr_factor):
    ...
    optimizer = torch.optim.Adam(
        [
            {
                "params": generative_model._prior.parameters(),
    ...
    for _ in range(num_data):
        while True:
            x = torch.stack(
                [
                    torch.randint(
                        num_primitives, (memory_size, num_arcs), device=device
                    ),
...
def train_sleep(
    generative_model, inference_network, num_samples, num_iterations, log_interval
):
    ...
    if generative_model.use_alphabet:
        alphabet = (
            torch.distributions.OneHotCategorical(
                logits=torch.ones(50, device=device).float()
            )
            .sample((num_samples,))
            .contiguous()
        )
        latent, obs = generative_model.sample_latent_and_obs(
            alphabet=alphabet, num_samples=1
        )
        latent = latent[0]
        obs = obs[0]
    else:
...
def get_mws_loss(
    generative_model, inference_network, memory, obs, obs_id, num_particles
):
    """
    Args:
    ...
    log_q = inference_network.get_log_prob_from_latent_dist(
        latent_dist, sampled_memory_latent
    )[
        0
    ]  # [batch_size]

    prior_loss = (
        -torch.gather(memory_log_prior, 0, sampled_memory_id[None, :]).mean().detach()
    )
