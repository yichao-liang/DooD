...
from dood import evaluate, losses, plot, util
from dood.models import air, base

# from models.mws.handwritten_characters.losses import get_mws_loss


def train(
    model, optimizer, scheduler, stats, data_loader, args, writer, dataset_name=None
):
    ...
    if args.model_type != "MWS":
        ...
        mws_transform = transforms.Resize([args.img_res, args.img_res], antialias=True)
    ...
    while iteration < args.num_iterations:
        ...
        with torch.no_grad():
            ...
            # evaluate.stroke_mll_plot(model, val_loader, args, writer, epoch)
        ...
        for imgs, target in train_loader:
            ...
            optimizer.zero_grad()
            loss_tuple = get_loss_tuple(
                args, generative_model, guide, iteration, imgs, writer, obs_id, memory
            )
...
def test_model(
    model, stats, test_loader, args, save_imgs_dir=None, epoch=None, writer=None
):
    evaluate.marginal_likelihoods(
        model,
        stats,
        test_loader,
        args,
...
def get_model_named_params(args, guide, generative_model):
    ...
    if args.model_type == "Sequential" and args.prior_dist == "Sequential":
        ...
        if not args.use_canvas and args.prior_dist == "Sequential":
            ...
            named_params = itertools.chain(
                guide.named_parameters(), generative_model.named_parameters()
            )
...
def get_loss_tuple(
    args, generative_model, guide, iteration, imgs, writer, obs_id, memory=None
):
    if args.model_type == "Base":
        base.schedule_model_parameters(
            generative_model, guide, iteration, args.loss, args.device
        )
...
def get_mws_loss_tuple(
    ...
    loss, theta_loss, phi_loss, prior_loss, accuracy, novel_proportion, new_map = (
        get_mws_loss(
            generative_model,
            guide,
            memory,
    ...
    return MWSLoss(
        neg_elbo=loss,
        theta_loss=torch.tensor(theta_loss),
        ...
        if args.model_type == "MWS":
            util.logging.info(
                ...
                "| last log_p = {} | last kl = {} | GPU memory = {:.2f} MB".format(
                    ...
                    (
                        loss_tuple.novel_proportion * 100
                        if loss_tuple.novel_proportion is not None
                        else None
                    ),
                    (
                        loss_tuple.new_map * 100
                        if loss_tuple.new_map is not None
                        else None
                    ),
            ...
            util.logging.info(
                f"Iteration {iteration} | Loss = {stats.trn_losses[-1]:.3f}"
            )
    return stats
