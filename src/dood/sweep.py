"""Train all the selected models Test them on the test suite Output the results in a
pdf."""

import argparse
import subprocess

from dood import exp_dict as ed
...
def get_args_parser():
    ...
    parser.add_argument(
        "--final_beta", default=1, type=float, help="Minimal value for the beta"
    )
    parser.add_argument("-ct", action="store_true", help="continue training")
    parser.add_argument(
        "-it",
        default=-1,
        type=int,
        help="""iteration to run test on, default -1
                        means latest""",
    )
    parser.add_argument("-m", default="mn", type=str, help="model code")
    parser.add_argument("-ds", default="mn", type=str, help="dataset name")
    parser.add_argument(
        "-trn",
        "--train_not_test",
        action="store_true",
...
if __name__ == "__main__":
    ...
    for n, args in all_exp_args.items():
        ...
        if train:
            ...
            args.extend(
                [
                    ...
                    model_name,
                    "--tb_dir",
                    f"/om/user/ycliang/log/hyper/{tb_name}",
        ...
        if evaluate:
            ...
            ckpt_path = util.get_checkpoint_path_from_path_base(model_name, run_args.it)
            subprocess.run(
                [
                    "python",
                    "-m",
