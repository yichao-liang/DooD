...
def main(args):
    ...
    if not (args.continue_training and Path(checkpoint_path).exists()):
        ...
        model, optimizer, scheduler, stats, data_loader = util.init(args, device)
    else:
        model, optimizer, scheduler, stats, data_loader, args = util.load_checkpoint(
            checkpoint_path, device, finetune_dataset_name=args.dataset
        )
    ...
    if args.model_type == "MWS":
        ...
        if not Path(checkpoint_path).is_file():
            mws.train.train_sleep(
                generative_model,
                guide,
                min(500, mws_args.batch_size * mws_args.num_particles),
...
def get_args_parser():
    ...
    # Model
    parser.add_argument(
        "--save_model_name", default="full-test", type=str, help="name for ckpt dir"
    )
    parser.add_argument(
        "--model_type",
    ...
    parser.add_argument("--z_dim", default=10, type=int, help="Only for VAE/AIR")
    parser.add_argument(
        "--num_mlp_layers",
        default=2,
        type=int,
    ...
    parser.add_argument(
        "--no_spline_renderer", action="store_true", help="if not specified then False"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no_maxnorm", action="store_true", help="if specified then True."
    )
    parser.add_argument(
        "--no_sgl_strk_tanh", action="store_true", help="if specified then True."
    )
    parser.add_argument(
        "--no_add_strk_tanh", action="store_true", help="if specified then True."
    )
    parser.add_argument(
        "--simple_pres",
        action="store_true",
    ...
    parser.add_argument(
        "--num_mixtures", default=1, help="num mixtures for the z_where, z_what GMM"
    )
    parser.add_argument(
        "--correlated_latent",
        action="store_true",
    ...
    parser.add_argument(
        "--feature_extractor_type", default="CNN", choices=["CNN", "MLP"]
    )
    ...
    parser.add_argument(
        "--beta", default=1, type=float, help="beta term as in beta-VAE"
    )
    # parser.add_argument("--final_bern", default=.5, type=float,
    #                     help="Minimal value for the z_pres Bern param")
    parser.add_argument(
        "--anneal_lr", action="store_true", help="if not specified then False"
    )
    parser.add_argument(
        "--increase_beta", action="store_true", help="if not specified then False"
    )
    parser.add_argument(
        "--log_grad", action="store_true", help="store gradient values in tensorboard"
    )
    parser.add_argument(
        "--log_param",
        action="store_true",
